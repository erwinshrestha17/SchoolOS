...
const NEPAL_DATE_TIME_FORMATTER = new Intl.DateTimeFormat(
...
export function toNepalLocalDateTime(
...
export function toBsDateFromGregorian(
  value: GregorianDate | Date | string | number,
): BsDate {
  ...
  if (remaining < 0) {
    throw new Error(
      "Gregorian date is before the supported BS conversion range.",
    );
  }
  ...
  if (year > BS_CALENDAR_MAX_YEAR) {
    throw new Error(
      "Gregorian date is after the supported BS conversion range.",
    );
  }
...
export function formatBsDateForInput(
  value: GregorianDate | Date | string | number,
): string {
...
export function formatBsDateRange(
  start: BsDate | string,
  end: BsDate | string,
): string {
...
export function daysInBsMonth(year: number, month: number): number {
  ...
  if (!lengths || month < 1 || month > 12) {
    throw new Error(
      `BS month is outside the supported ${BS_CALENDAR_MIN_YEAR}-${BS_CALENDAR_MAX_YEAR} conversion range.`,
    );
  }
...
export function isSameNepalSchoolDay(
...
export function getNepalSchoolDay(
  ...
  return {
    ...
    startUtc: zonedNepalDateTimeToUtc({
      ...gregorianDate,
      hour: 0,
      minute: 0,
      second: 0,
...
export function shiftGregorianDateOnly(value: string, days: number): string {
  ...
  if (!match || !Number.isInteger(days)) {
    throw new Error(
      "A valid Gregorian date and whole-day offset are required.",
    );
  }
...
export function zonedNepalDateTimeToUtc(
  ...
  [hour, minute, second, millisecond].forEach((part, index) =>
    assertFiniteInteger(
      part,
      ["hour", "minute", "second", "millisecond"][index],
    ),
  );
...
function totalDaysInBsYear(year: number): number {
  ...
  if (!lengths) {
    throw new Error(
      `BS year is outside the supported ${BS_CALENDAR_MIN_YEAR}-${BS_CALENDAR_MAX_YEAR} conversion range.`,
    );
  }
...
function toInstant(value: Date | string | number): Date {
  ...
  if (Number.isNaN(date.getTime())) {
    throw new Error(
      "Date value must be a valid ISO-8601 UTC timestamp or Date instance.",
    );
  }
  return date;
}
...
function getNepalOffsetMilliseconds(value: Date): number {
  const part = NEPAL_OFFSET_FORMATTER.formatToParts(value).find(
    (item) => item.type === "timeZoneName",
  )?.value;
  const match = /^GMT([+-])(\d{1,2})(?::?(\d{2}))?$/.exec(part ?? "");
  if (!match) {
    throw new Error(
      `Unable to resolve ${NEPAL_TIME_ZONE} from the runtime IANA timezone database.`,
    );
  }
