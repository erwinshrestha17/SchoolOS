...
- **Bottom-nav item counts — corrected after live-device testing.** Teacher and Principal shells use exactly 5 primary items, matching the brief. **Parent does not: it has 6 live bottom-nav items** (Home, Children, Attendance, Homework, Notices, More), exceeding the brief's own "avoid more than five" rule and not matching its recommended Parent structure (5 items, no separate Notices tab). This was **missed by static code review** — I initially (incorrectly) concluded via `grep` that a 6-item nav list (`RoleShellScaffold._itemsForRole('PARENT')`) was unreachable dead code, since that specific class is indeed never invoked with `role: 'PARENT'`. What I missed: `SchoolOsAppShell` has its **own separate** custom bottom-nav widget, `SchoolOsBottomNavigation` (in `school_os_app_shell.dart`), with a hardcoded 6-item list — a different code path entirely, and very much live. Confirmed by walking the Parent persona on a real device: all 6 tabs render and work (Attendance pushes to a standalone screen rather than being an `IndexedStack` tab, which is why the shell's own `titles` list only has 5 entries — that list is just the app-bar title per tab, not the bottom-nav item count). **This needs a product decision, not a code fix**: which item to cut/merge to get to 5, or whether 6 is acceptable here.
- **`RoleShellScaffold._itemsForRole('PARENT')` dead-code claim was also wrong — corrected and fixed 2026-07-11.** The line above originally called this "a separate, smaller, still-valid finding — safe to delete." It wasn't: re-verifying before deleting turned up three live screens still constructing `RoleShellScaffold(role: 'PARENT', ...)` — `ParentTimetableScreen` (reachable via More → Subject Timetable), `LearningSummaryScreen` (More → Learning Summary), and `ParentActivityScreen` (push-notification deep link only). All three rendered the mismatched Material `NavigationBar` shell instead of the pill-style `SchoolOsBottomNavigation` every sibling Parent detail screen (`ParentDetailScaffold`) uses — a real, live visual inconsistency, not dead code. Fixed by migrating all three screens to `ParentDetailScaffold`. A fourth screen using the same `RoleShellScaffold(role: 'PARENT', ...)` pattern, `ChildProfileScreen`, had zero references anywhere (not even a deep link) and was genuinely dead — deleted outright. With no remaining callers, `RoleShellScaffold._itemsForRole('PARENT')` is now actually safe to delete and has been removed.
...
**One thing still needs a decision from you, not more code archaeology:**
1. **Parent's bottom nav has 6 items, not 5** (Section 5) — my Phase 1 conclusion that this was dead code was wrong; it's live and violates the brief's own nav-count guidance.

Resolved since Phase 1: the parent-teacher chat feature (Section 0) has been removed entirely — it contradicted the brief's "Do not add" list. The `RoleShellScaffold._itemsForRole('PARENT')` dead-code claim was also wrong at the time (three live screens still used it); those screens were migrated to `ParentDetailScaffold` for nav consistency, a fourth (`ChildProfileScreen`) was confirmed genuinely dead and deleted, and the now-truly-unused `PARENT` case was removed (Section 5).

Smaller open items: the stale "Needs mobile timetable DTO" label, whether Principal's raw-JSON-map pattern is intentional.
