...
const NEXT_STATIC_DESTINATIONS = new Set(['script', 'style', 'font', 'worker']);
...
function isForbiddenFromCache(request, url) {
  return (
    ...
    (request.destination === 'image' && !PUBLIC_ASSET_PATHS.has(url.pathname))
  );
}
...
self.addEventListener('fetch', (event) => {
  ...
  if (
    isExplicitPublicAsset(request, url) ||
    isImmutableNextStatic(request, url)
  ) {
