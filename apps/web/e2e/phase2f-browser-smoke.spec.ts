...
test.describe('Phase 2F.2 authenticated school admin browser smoke', () => {
  ...
  test('loads polished daily school workflows without fatal pages', async ({
    page,
  }) => {
    const targets = [
      {
      ...
      { route: '/dashboard/students', visible: /Student Directory/i },
      {
        ...
        visible: /Admissions|New admission|Applications Needing Review/i,
      },
      {
        route: '/dashboard/attendance',
        visible: /Attendance Roster|Smart Attendance/i,
      },
      {
        route: '/dashboard/fees/collect',
        visible: /Collect payment|Find student or invoice/i,
      },
      {
        route: '/dashboard/academics',
        visible: /Academics|Active Exam Terms|Workflow Readiness/i,
      },
      {
      ...
      },
      {
        ...
        visible:
          /Choose a settings area|School setup status|Recent configuration changes/i,
      },
    ] as const;
    ...
    for (const target of targets) {
      ...
      await expect(
        page.locator('main').getByText(target.visible).first(),
      ).toBeVisible({
        timeout: 15_000,
      });
  ...
  test('student directory filters and profile link are safe with seeded data', async ({
    ...
    await page
      .getByLabel(
        /Search students by name, student code, guardian name, or phone/i,
      )
      .fill('STU001');
  ...
  test('attendance and admissions validation controls render', async ({
    page,
  }) => {
    ...
    await page
      .getByRole('button', {
        name: 'Start school-office admission',
        exact: true,
      })
      .click();
    await expect(
    ...
    await expect(
      page.getByText('Enter valid student names.', { exact: true }),
    ).toBeVisible();
  });

  test('platform routes are denied or redirected for a school user', async ({
    page,
  }) => {
...
test.describe('Phase 2F.2 platform browser smoke', () => {
  ...
  test('loads platform pilot onboarding surfaces for platform users', async ({
    ...
    for (const route of [
      ...
      await expect(
        page
          .locator('h1:visible, h2:visible, [role="heading"]:visible')
          .first(),
...
async function login(
  ...
  await Promise.race([
    ...
    expect(
      page
        .getByText(/Invalid credentials|Account suspended|not authorized/i)
        .first(),
  ...
  if (!page.url().includes('/dashboard') && !page.url().includes('/platform')) {
    throw new Error(
      `Login failed for ${credentials.email}. Still at ${page.url()}`,
    );
  }
