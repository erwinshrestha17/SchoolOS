...
test.describe('Platform Control Plane Smoke Tests', () => {
  test.beforeEach(async ({ page }) => {
    ...
    const tenantSlug =
      process.env.SCHOOLOS_E2E_PLATFORM_TENANT_SLUG || 'default-school';
    const email =
      process.env.SCHOOLOS_E2E_PLATFORM_EMAIL || 'platform@schoolos.com';
    const password =
      process.env.SCHOOLOS_E2E_PLATFORM_PASSWORD || 'platform123';
  ...
  test('Access Platform Dashboard and check metrics', async ({ page }) => {
    ...
    await expect(
      page.getByText('Control Plane', { exact: true }),
    ).toBeVisible();
  ...
  test('Navigate to School Directory and search', async ({ page }) => {
    ...
    const searchInput = page.locator(
      'input[placeholder*="Search school name"]',
    );
  ...
  test('Verify Tenant Isolation - Restricted access for standard admin', async ({
    page,
    context,
  }) => {
    // Logout superadmin
  ...
  test('Tenant Change Plan workflow renders with guarded submit', async ({
    page,
  }) => {
    await page.goto('/platform/schools');
    await page.click('text=Trial Academy');
    ...
    await expect(
      page.getByText(/SchoolOS subscription billing only/i),
    ).toBeVisible();

    const submit = page.getByRole('button', { name: /Change plan/i });
    await expect(submit).toBeDisabled();
    await page
      .getByPlaceholder(/School upgraded/i)
      .fill('Pilot upgrade approved');
    await expect(submit).toBeEnabled();
  });

  test('Tenant detail operator dialogs render for support, billing, invoices, and onboarding', async ({
    page,
  }) => {
    await page.goto('/platform/schools');
    await page.click('text=Trial Academy');
    ...
    await expect(
      page.getByRole('heading', { name: /Enter support mode/i }),
    ).toBeVisible();
    const supportSubmit = page
      .getByRole('button', { name: /Enter support mode/i })
      .last();
    await expect(supportSubmit).toBeDisabled();
    await page
      .getByPlaceholder(/support case/i)
      .fill('Support ticket verification');
    ...
    await expect(
      page.getByRole('heading', {
        name: /Create SchoolOS subscription invoice/i,
      }),
    ).toBeVisible();
    await page
      .getByRole('button', { name: 'Cancel', exact: true })
    ...
    await expect(
      page.getByRole('heading', { name: /Edit billing profile/i }),
    ).toBeVisible();
    await page
      .getByRole('button', { name: 'Cancel', exact: true })
      .first()
  ...
  test('Platform settings deep links and operator dialogs render', async ({
    page,
  }) => {
    ...
    await expect(page.getByRole('tab', { name: 'Queues' })).toHaveAttribute(
      'data-state',
      'active',
    );
    ...
    if (await retryAll.isEnabled()) {
      ...
      await expect(
        page.getByRole('heading', { name: 'Retry Failed Job' }),
      ).toBeVisible();
      await page.getByRole('button', { name: 'Cancel' }).click();
    }
    ...
    await expect(
      page.getByRole('tab', { name: 'Global Audit' }),
    ).toHaveAttribute('data-state', 'active');
    await expect(page.getByText('Resource ID').first()).toBeVisible();
    await expect(
      page.getByText('Export current page CSV').first(),
    ).toBeVisible();
  });
  ...
  test('Feature Override Workflow', async ({ page }) => {
    ...
    const transportToggle = page
      .getByText(/M9 Transport/i)
      .locator('xpath=../..')
      .getByRole('button');
    ...
    await expect(
      page.getByRole('heading', { name: /Confirm module override/i }),
    ).toBeVisible();
  ...
  test('Queue Job Inspection safety', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: /Failed Jobs/i }),
    ).toBeVisible();
    await expect(
      page.locator('text=Sensitive data in payloads is masked'),
    ).toBeVisible();
  });
});
