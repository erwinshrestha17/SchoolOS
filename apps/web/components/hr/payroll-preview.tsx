      ...
      <div className="flex items-start gap-4 p-4 bg-amber-50 rounded-2xl border border-amber-100">
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-amber-800 leading-relaxed">
            This is a draft payroll calculation for informational purposes. No
            accounting entries, salary slips, or payroll runs are created from
            this screen. To process payroll, go to the{' '}
            <strong>Payroll Approval</strong> workflow.
          </p>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse text-sm">
            <thead>
              <tr className="bg-gray-50/50 border-b border-gray-100">
                ...
                <th className="px-6 py-4 font-semibold text-[var(--color-mod-hr-text)] text-right">
                  Net Preview Pay
                </th>
              </tr>
            </thead>
              ...
              {previewQuery.isLoading ? (
                Array.from({ length: 5 }).map((_, i) => (
                  <tr key={i} className="animate-pulse">
                    ...
                    <td className="px-6 py-4">
                      <div className="h-4 w-16 ml-auto bg-gray-100 rounded" />
                    </td>
                    <td className="px-6 py-4">
                      <div className="h-4 w-16 ml-auto bg-gray-100 rounded" />
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="h-4 w-12 mx-auto bg-gray-100 rounded" />
                    </td>
                    <td className="px-6 py-4">
                      <div className="h-4 w-16 ml-auto bg-gray-100 rounded" />
                    </td>
                    <td className="px-6 py-4">
                      <div className="h-4 w-20 ml-auto bg-gray-100 rounded" />
                    </td>
                  </tr>
                ))
              ) : previewQuery.error ? (
                <tr>
                  ...
                  >
                    Failed to load payroll preview. Please check your connection
                    or permissions.
                  </td>
                </tr>
                      ...
                      {row.warnings.length > 0 && (
                        <div className="mt-1 flex flex-col gap-0.5">
                          {row.warnings.map((w, i) => (
                            <span
                              key={i}
                              className="text-[9px] text-amber-600 font-medium flex items-center gap-1"
                            >
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="flex flex-col items-center">
                        ...
                        <div className="flex gap-1 mt-1">
                          <span className="px-1.5 py-0.5 rounded bg-success-50 text-success-700 text-[9px] font-bold">
                            {row.presentDays}P
                          </span>
                          {row.approvedPaidLeaveDays > 0 && (
                            <span className="px-1.5 py-0.5 rounded bg-[var(--color-mod-hr-soft)] text-[var(--color-mod-hr-text)] text-[9px] font-bold">
                              {row.approvedPaidLeaveDays}L
                            </span>
                          )}
                          {row.unpaidLeaveDays > 0 && (
                            <span className="px-1.5 py-0.5 rounded bg-danger-50 text-danger-700 text-[9px] font-bold">
