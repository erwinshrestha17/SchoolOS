...
export function PayslipList() {
  ...
  const payslipColumns: PaginatedDataTableColumn<PayslipSummary>[] = [
    {
      id: 'payslipNumber',
      header: 'Payslip #',
      cell: (payslip) => (
  ...
  return (
    <div className="space-y-6">
      ...
      <PaginatedDataTable
        ...
        errorMessage="Failed to load payslips. Please check your permissions."
        emptyTitle="No payslips yet"
        emptyDescription="Payslips will appear here once a payroll run generates them."
        hasActiveFilters={Boolean(search)}
        noResultsTitle="No matching payslips"
          ...
          <div className="flex flex-wrap justify-end gap-2">
            {canRegeneratePayslips ? (
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-sm font-bold text-amber-800 transition-all hover:bg-amber-100 disabled:cursor-not-allowed disabled:opacity-60"
              >
                ...
                {isRegenerationPendingFor(payslip.id) ? 'Queued' : 'Regenerate'}
              </button>
            ) : null}
            <button
              type="button"
