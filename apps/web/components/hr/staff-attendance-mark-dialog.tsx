...
export function StaffAttendanceMarkDialog({
  isOpen,
  onClose,
}: StaffAttendanceMarkDialogProps) {
        ...
        <DialogHeader className="flex justify-between items-center pr-12">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              Submit bulk attendance logs for active staff.
            </p>
          </div>
          <button
            type="button"
                ...
                {staffRosterQuery.isLoading ? (
                  Array.from({ length: 4 }).map((_, idx) => (
                    <tr key={idx} className="animate-pulse">
                      ...
                      <td className="px-5 py-4">
                        <div className="h-6 w-44 bg-slate-100 rounded-full" />
                      </td>
                      <td className="px-5 py-4">
                    ...
                    >
                      ...
                      <td className="px-5 py-4">
                        <div className="flex gap-1.5 p-1 bg-slate-100 rounded-xl w-fit">
                          {(
                            ['PRESENT', 'LATE', 'ABSENT', 'HALF_DAY'] as const
                          ).map((st) => (
                            <button
                              key={st}
                              type="button"
                              onClick={() =>
                                handleStatusChange(row.staffId, st)
                              }
                              className={cn(
                      ...
                      <td className="px-5 py-4">
                        <Input
                          ...
                          onChange={(e) =>
                            handleTimeChange(row.staffId, e.target.value)
                          }
                      ...
                      <td className="px-5 py-4">
                        <Input
                          ...
                          value={row.note}
                          onChange={(e) =>
                            handleNoteChange(row.staffId, e.target.value)
                          }
                          placeholder="Note"
                          className="h-9 py-1 text-xs rounded-xl"
                        />
