        ...
        <thead>
          <tr className="bg-slate-50">
            ...
            {daysOfWeek.map((day) => (
              <th
                key={day.value}
                className="border-b border-slate-200 p-6 text-center min-w-[200px]"
              >
                ...
                const slot = timetable.find(
                  (s) =>
                    ...
                    (s.periodId === period.id ||
                      (s.startsAt === period.startsAt &&
                        s.endsAt === period.endsAt)),
                );
                const subjectName =
                  slot?.subject?.name?.trim() || 'Subject not set';
                const staffName =
