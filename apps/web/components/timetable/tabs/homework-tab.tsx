...
function formatHomeworkDateTime(
  value: string | null | undefined,
  fallback = "Date not set",
) {
  if (!value) return fallback;
...
export function HomeworkTab({
  ...
  const homeworkQuery = useQuery({
    ...
    queryFn: () =>
      api.listHomework({ classId, status: statusFilter || undefined }),
  });
  ...
  const selectedAssignment = homeworkQuery.data?.find(
    (homework) => homework.id === selectedAssignmentId,
  );
  const assignmentSubmissions =
    submissionsQuery.data?.filter(
      (submission) => submission.homeworkId === selectedAssignmentId,
    ) ?? [];
          ...
          {homeworkQuery.isLoading ? (
            ...
            <PageState
              ...
              description={
                ...
                "Homework assignments could not be loaded."
              }
            />
            ...
            <div className="space-y-4">
              {homeworkQuery.data?.map((h) => {
                ...
                const subjectName =
                  h.subject?.name?.trim() || "Subject not set";
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex flex-wrap gap-2 pt-2">
                    ...
                    <button
                      ...
                      onClick={() =>
                        closeMutation.mutate(selectedAssignment.id)
                      }
                      disabled={
                ...
                {submissionsQuery.isLoading ? (
                  ...
                  <PageState
                    ...
                    description={
                      ...
                      "Homework submissions could not be loaded."
                    }
                  />
                  ...
                  <div className="space-y-4">
                    {assignmentSubmissions.map((s) => (
                      <div
                        key={s.id}
                        className="group space-y-6 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm"
                      >
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
                                {s.submittedAt
                                  ? `Submitted · ${formatHomeworkDateTime(s.submittedAt)}`
                                  : "Not submitted"}
                              </p>
                            </div>
                          </div>
                        ...
                        {s.attachments && s.attachments.length > 0 && (
                          <div className="space-y-3">
                            <AuditInfo>
                              Attachments are shown from backend file metadata.
                              Use signed preview/download endpoints when
                              available; permanent public URLs are intentionally
                              not exposed here.
                            </AuditInfo>
