...
export function TransportWorkspace({ initialTab = 'overview' }: TransportWorkspaceProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6">
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            <StatCard title="Route assignments" value={stats.totalRoutes} icon={<Route size={18} />} loading={reportsQuery.isLoading} />
            <StatCard title="Active vehicles" value={stats.activeVehicles} icon={<Bus size={18} />} loading={vehiclesQuery.isLoading} />
            <StatCard title="Active trips" value={stats.activeTrips} icon={<Navigation size={18} />} loading={activeTripsQuery.isLoading} />
            <StatCard title="Assigned students" value={stats.assignedStudents} icon={<Users size={18} />} loading={studentsQuery.isLoading} />
            <StatCard title="Logs today" value={stats.logsToday} icon={<MapPin size={18} />} loading={reportsQuery.isLoading} />
            <StatCard title="Expiry alerts" value={stats.alerts} icon={<AlertTriangle size={18} />} loading={reportsQuery.isLoading} />
          </div>
          <InfoCard title="Privacy rules" lines={['Parents will only see their own child\'s assigned vehicle/trip in the future parent view.', 'Driver app will only expose trips assigned to that driver later.', 'Live map/WebSocket tracking is intentionally deferred; this admin slice shows latest coordinates only.']} />
                ...
                <div key={route.id} className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-sm text-slate-500">{route.code} • {route.isActive ? 'Active' : 'Inactive'}</p>
                    </div>
            ...
            <div className="space-y-3">
              {vehicles.map((vehicle) => (
                <div key={vehicle.id} className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                  ...
                  <p className="text-sm text-slate-500">{vehicle.model || 'Model not set'} • {vehicle.capacity} seats • {vehicle.status}</p>
                </div>
              ))}
            </div>
          ...
          <Panel title="Assignments" description="Assign drivers and students to operational transport routes.">
            ...
            <div className="mt-3 space-y-3">
              {studentAssignments.map((assignment) => (
                <RecordCard key={assignment.id} title={studentLabel(assignment.student) || assignment.studentId} subtitle={`${assignment.route?.name ?? assignment.routeId} • ${assignment.stop?.name ?? assignment.stopId}`} />
              ))}
            </div>
          </Panel>
      ...
      {activeTab === 'trips' && (
        <TwoColumn>
          <Panel title="Trips" description="Start, complete, and mark student transport status.">
            ...
            <div className="mt-5">
              <h3 className="text-sm font-bold text-slate-700">Recent trips</h3>
              <TripList trips={trips.slice(0, 6)} emptyTitle="No trip history" compact />
            </div>
          </Panel>
        ...
        <div key={trip.id} className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-slate-500">{trip.vehicle?.registrationNumber ?? trip.vehicleId} • {trip.direction} • {trip.status}</p>
...
function RecordCard({ title, subtitle }: { title: string; subtitle: string }) {
  return (
