...
export function AttendanceConflictReview({ conflicts }: AttendanceConflictReviewProps) {
    ...
    >
      <div className="space-y-4">
        {conflicts.map((conflict) => (
          <div key={conflict.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-5 bg-white border border-slate-100 rounded-2xl transition-all hover:border-slate-200">
            <div className="mb-4 sm:mb-0">
              <div className="flex items-center gap-2 mb-1">
                <p className="font-bold text-slate-900">
                  {conflict.className ?? 'Class'}{conflict.sectionName ? ` / ${conflict.sectionName}` : ''}
                </p>
            ...
            {!conflict.reviewedAt && (
              <button
                onClick={() => reviewMutation.mutate({
                  id: conflict.id,
                  resolutionNote: 'Reviewed and resolved via dashboard review queue.'
                })}
              ...
              >
                {reviewMutation.isPending ? 'Resolving...' : (
                  <>
        ...
        {conflicts.length === 0 && (
          <div className="text-center py-8">
            ...
            <p className="text-sm font-bold text-slate-900">No Pending Conflicts</p>
            <p className="text-xs text-slate-500 mt-1">All attendance sessions are consistent.</p>
          </div>
        )}
      </div>
    </SectionCard>
