    ...
    <div className="mt-5 border-t border-slate-100 pt-5">
      ...
      <div className="space-y-2">
        {items.slice(0, 5).map((item) => {
          const href = APPROVED_DASHBOARD_ROUTES.has(item.action)
            ? item.action
            : null;
          const content = (
            <>
...
export function SummaryUnavailableState({
  status,
}: {
  status: OperationalSummaryStatus;
}) {
...
export function OperationalSummaryLoading() {
  return (
    <LoadingState variant="skeleton" label="Loading operational summary..." />
  );
}
