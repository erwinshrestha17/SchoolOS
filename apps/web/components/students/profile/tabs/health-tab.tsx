        ...
        <div className="grid gap-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="font-bold text-slate-900">
                  {student.emergencyName || 'Not recorded'}
                </p>
                <p className="text-sm text-slate-500 font-medium">
                  {student.emergencyPhone || 'No phone recorded'}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-slate-100 bg-slate-50/50 p-5">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="font-bold text-slate-900">
                  {student.doctorName || 'Not recorded'}
                </p>
                <p className="text-sm text-slate-500 font-medium">
                  {student.doctorPhone || 'No phone recorded'}
                </p>
              </div>
            </div>
          </div>
...
function HealthItem({
  ...
  label: string;
  value: string;
  tone: 'danger' | 'warning' | 'admissions' | 'neutral';
}) {
  const colors = {
    danger: 'text-danger-500 bg-danger-50',
    warning: 'text-warning-500 bg-warning-50',
    admissions:
      'text-[var(--color-mod-admissions-accent)] bg-[var(--color-mod-admissions-bg)]',
  ...
  return (
    <div className="flex items-start gap-4">
      <div
        className={`mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${colors[tone]}`}
      >
        {icon}
      </div>
      <div>
        ...
        <p
          className={`mt-0.5 font-bold ${tone === 'neutral' ? 'text-slate-500' : 'text-slate-900'}`}
        >
          {value}
        </p>
      </div>
