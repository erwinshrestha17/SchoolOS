...
export function AdmissionFollowUpsCard({ studentId }: { studentId: string }) {
  ...
  if (query.isLoading) {
    return (
      <div
        id="admission-follow-ups"
        className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-white p-4 text-sm font-semibold text-slate-600"
      >
        <Loader2 className="h-4 w-4 animate-spin" />
        Loading admission follow-ups…
      </div>
    );
  }
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">
            These items do not block the student’s active record.
          </p>
        </div>
        <span className="rounded-full bg-blue-50 px-2.5 py-1 text-xs font-bold text-blue-800">
          {query.data.items.length} to review
        </span>
      </div>
      <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        {query.data.items.map((item) => {
          const Icon =
            item.code === 'DOCUMENTS_PENDING'
              ? FileText
