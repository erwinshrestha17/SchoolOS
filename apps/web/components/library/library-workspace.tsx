...
export function LibraryWorkspace({
  ...
  const [issueForm, setIssueForm] =
    useState<LibraryIssuePayload>(emptyIssueForm);
  ...
  const [returnDrafts, setReturnDrafts] = useState<
    Record<string, ReturnLibraryIssuePayload>
  >({});
  const [notice, setNotice] = useState<string | null>(null);
  const [isConfirmingReturn, setIsConfirmingReturn] = useState<string | null>(
    null,
  );
  ...
  const updateBookMutation = useMutation({
    mutationFn: ({
      ...
      id: string;
      body: Partial<LibraryBookPayload>;
  ...
  const updateCopyMutation = useMutation({
    mutationFn: ({
      ...
      id: string;
      body: Partial<LibraryCopyPayload>;
    ...
    <div className="space-y-6">
      ...
      {activeTab === "copies" && (
        <CopiesPanel
          ...
          isSaving={
            createCopyMutation.isPending || updateCopyMutation.isPending
          }
          error={
      ...
      >
        <div className="mt-4 max-h-[60vh] overflow-y-auto space-y-4">
          ...
          {historyQuery.data?.history.map((issue: any) => (
            <div
              key={issue.id}
              className="p-3 rounded-xl border border-slate-100 bg-slate-50 text-sm"
            >
          ...
          {historyQuery.data?.history.length === 0 && (
            <EmptyState
              title="No history"
              description="This item has never been issued."
            />
          )}
        </div>
      </ConfirmDialog>
        ...
        <div className="space-y-3">
          ...
          {overdueIssues.length === 0 && (
            <EmptyState
              ...
              description="Library circulation is currently clear."
            />
          )}
        </div>
      </WorkSurface>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          ...
          >
            ...
            <p className="mt-1 text-sm text-slate-500">
              Popular books, damaged inventory, and CSV exports.
            </p>
          </Link>
          <Link
          ...
          >
            ...
            <p className="mt-1 text-sm text-slate-500">
              Track pending fines and apply audit-logged waivers.
            </p>
          </Link>
          <Link
            href="/dashboard/library/issues"
          ...
          >
            ...
            <p className="mt-1 text-sm text-slate-500">
              Search student history and check active issue limits.
            </p>
          </Link>
        </div>
      </WorkSurface>
...
function BooksPanel(props: {
  ...
  const visibleBooks = props.books.filter(
    (book) =>
      (!category || book.subjectCategory === category) &&
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-3">
          <SearchBox
            ...
            placeholder="Search catalogue (Title, ISBN, Author)"
          />
          <select
            value={author}
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <form onSubmit={props.onSubmit} className="mt-5 space-y-4">
          <TextInput
            ...
            required
            onChange={(title) => props.setForm({ ...props.form, title })}
          />
          <TextInput
            label="Author"
            value={props.form.author}
            required
            onChange={(author) => props.setForm({ ...props.form, author })}
          />
          <TextInput
            label="ISBN"
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <TextInput
              ...
              onChange={(value) =>
                props.setForm({
                  ...props.form,
                  publishedYear: value ? Number(value) : undefined,
                })
              }
            />
            <TextInput
          ...
          <div className="flex gap-2">
            ...
            {props.editingBookId && (
              <button
                ...
                className="btn-secondary"
                onClick={() => {
                  props.setEditingBookId(null);
                  props.setForm(emptyBookForm);
                }}
              >
                Cancel
              </button>
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_420px]">
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <PanelHeader
          ...
          description="Track physical copies, barcodes, shelf location, and circulation status."
        />
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <form onSubmit={props.onSubmit} className="mt-5 space-y-4">
          <label className="block text-sm font-semibold text-slate-700">
            ...
            <select
              required
              value={props.form.bookId}
              onChange={(e) =>
                props.setForm({ ...props.form, bookId: e.target.value })
              }
          ...
          <TextInput
            ...
            required
            onChange={(barcode) => props.setForm({ ...props.form, barcode })}
          />
          <TextInput
            label="QR code"
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <TextInput
              ...
              onChange={(value) =>
                props.setForm({
                  ...props.form,
                  replacementCost: value ? Number(value) : undefined,
                })
              }
            />
            <TextInput
          ...
          <div className="flex gap-2">
            ...
            {props.editingCopyId && (
              <button
                ...
                className="btn-secondary"
                onClick={() => {
                  props.setEditingCopyId(null);
                  props.setForm(emptyCopyForm);
                }}
              >
                Cancel
              </button>
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_420px]">
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <PanelHeader
          title="Issue / Return"
          description="Issue available copies and close active borrower records."
        />
      ...
      <section className="space-y-6">
        <section className="rounded-2xl border border-[var(--color-mod-library-border)] bg-[var(--color-mod-library-bg)] p-5 shadow-sm">
          <PanelHeader
            title="Quick QR Lookup"
            description="Scan student QR to select the borrower and review borrowing status."
          />
          <QRResolver
            purpose="LIBRARY"
            autoFocus
          ...
          />
          {resolvedBorrower ? (
            <QrBorrowerSummary borrower={resolvedBorrower} />
          ) : null}
        </section>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <PanelHeader
            title="Issue copy"
            description="Select a physical copy and a borrower."
          />
          {props.error && <ErrorNotice message={props.error.message} />}
...
function ReservationsPanel(props: {
  ...
  setFulfillmentDrafts: (
    drafts: Record<string, FulfillLibraryReservationPayload>,
  ) => void;
  onSubmit: (event: React.FormEvent<HTMLFormElement>) => void;
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs font-semibold text-slate-500">
            Scan the physical copy code, then scan borrower QR or select
            manually.
          </p>
        </div>
  ...
  return (
    <div className="mt-4 rounded-2xl border border-[var(--color-mod-library-border)] bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-xs font-bold uppercase tracking-wide text-[var(--color-mod-library-text)]">
...
function IssueSummaryLine({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-start justify-between gap-3">
      ...
      <span className="min-w-0 text-right font-semibold text-slate-700">
        {value}
      </span>
    </div>
  );
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <PanelHeader
          ...
          description="Review overdue copies and queue consent-aware reminders."
        />
        <button
          type="button"
...
function cleanBookPayload(form: LibraryBookPayload): LibraryBookPayload {
  return {
    ...
    ...(form.subjectCategory
      ? { subjectCategory: form.subjectCategory.trim() }
      : {}),
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <PanelHeader
            title="Issued Books"
            description="Recent active circulation records included in the CSV export."
          />
          <div className="mt-5 space-y-3">
            ...
            {issuedBooks.length === 0 && !isLoading && (
              <EmptyState
                ...
                description="There are no active issued copies in this report."
              />
            )}
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <PanelHeader
            title="Overdue Books"
            description="Active issues past their due date."
          />
          <div className="mt-5 space-y-3">
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <PanelHeader
            title="Popular Books"
            description="Most frequently issued books in the library."
          />
          <div className="mt-5 space-y-3">
            ...
            {popularBooks.length === 0 && !isLoading && (
              <EmptyState
                title="No popularity data"
                description="Book issue history will appear here after circulation starts."
              />
            )}
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <PanelHeader
            title="Lost / Damaged Copies"
            description="Current inventory items marked as unusable."
          />
          <div className="mt-5 space-y-3">
            ...
            {lostDamaged.length === 0 && !isLoading && (
              <EmptyState
                title="No lost/damaged books"
                description="Inventory health is currently clear."
              />
            )}
          </div>
        </section>
