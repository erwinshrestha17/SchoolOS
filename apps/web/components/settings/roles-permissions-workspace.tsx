...
export function RolesPermissionsWorkspace() {
  ...
  const filteredRoles = useMemo(
    () =>
      (rolesQuery.data ?? []).filter(
        (role) =>
          !searchable ||
          `${role.name} ${role.permissions.map((permission) => permission.key).join(' ')}`
            .toLowerCase()
            .includes(searchable),
      ),
    [rolesQuery.data, searchable],
  );
  ...
  if (rolesQuery.isError || permissionsQuery.isError)
    return (
      <div className="p-6">
        <ErrorState
          title="Could not load roles and permissions"
          message="Please retry to load this school’s role catalog and permission coverage."
  ...
  return (
    <div className="space-y-6 p-6 pb-24">
      ...
      <SettingsPermissionNotice
        access="view-only"
        description="You can review role coverage here. Role assignment and permission changes remain in authorised user-access workflows."
      />
      <section className="grid gap-4 md:grid-cols-3">
        <Stat label="School roles" value={roles.length} icon={UsersRound} />
        <Stat
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col gap-4 border-b border-slate-100 p-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h2 className="font-bold text-slate-950">Role coverage</h2>
            <p className="mt-1 text-sm text-slate-600">
              Use this view when assigning user roles. Do not grant broad access
              where a focused role is sufficient.
            </p>
          </div>
          <label className="relative block w-full lg:w-80">
            ...
            <input
              ...
              className="h-10 w-full rounded-lg border border-slate-200 bg-white pl-9 pr-3 text-sm outline-none focus:border-slate-500 focus:ring-2 focus:ring-slate-900/5"
            />
          </label>
        </div>
        {filteredRoles.length === 0 ? (
          <div className="p-10 text-center">
            ...
            <p className="mt-1 text-sm text-slate-600">
              Try a different role or permission keyword.
            </p>
          </div>
        ) : (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-600">
                      {role.permissions.length} permission keys
                    </p>
                  </div>
