...
import { CheckCircle2, ImageUp, Save, ShieldCheck, Trash2 } from 'lucide-react';
import type {
...
type BrandingForm = Omit<
  BrandingDocumentsSettings,
  'logoFileAssetId' | 'updatedAt'
>;
...
export function BrandingDocumentsWorkspace() {
  ...
  const changed = useMemo(
    () =>
      brandingQuery.data
        ? JSON.stringify(form) !==
          JSON.stringify(withoutFile(brandingQuery.data))
        : false,
  ...
  if (brandingQuery.isError || !brandingQuery.data) {
    const denied =
      brandingQuery.error instanceof ApiRequestError &&
      brandingQuery.error.statusCode === 403;
    return (
      <ErrorState
        title={
          denied ? 'Permission denied' : 'Could not load Branding & Documents'
        }
        message={
          denied
            ? 'You do not have permission to manage school branding and official document defaults.'
            : undefined
        }
  ...
  const setValue = <K extends keyof BrandingForm>(
    key: K,
    ...
    <div className="space-y-6 p-6 pb-28">
      ...
      <section className="rounded-2xl border border-sky-100 bg-sky-50 p-5 text-sm text-sky-900">
        <div className="flex gap-3">
          ...
          <div>
            <p className="font-bold">Protected official files</p>
            <p className="mt-1">
              School logos are stored through File Registry. Open or download
              them only through authenticated protected-file actions.
            </p>
          </div>
        </div>
      </section>
      ...
      {saveMutation.isError ? (
        <ErrorState
          title="Could not save branding defaults"
      ...
      {removeMutation.isError ? (
        <ErrorState
          title="Could not remove the school logo"
          error={removeMutation.error}
          onRetry={() => removeMutation.mutate()}
          className="min-h-[180px]"
        />
      ) : null}
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-100 p-6">
          <h2 className="font-bold text-slate-950">Official school logo</h2>
          <p className="mt-1 text-sm text-slate-600">
            Used only where the approved document template supports a school
            logo.
          </p>
        </div>
        <div className="flex flex-col gap-5 p-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="font-semibold text-slate-900">
                {branding.logoFileAssetId
                  ? 'Official logo is uploaded'
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <ProtectedFileButton
              fileAssetId={branding.logoFileAssetId}
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <Section
          ...
          description="These values support approved SchoolOS document templates; they do not alter protected template files."
        >
          <label>
          ...
          <SelectField
            ...
            onChange={(value) =>
              setValue(
                'defaultPaperSize',
                value as BrandingForm['defaultPaperSize'],
              )
            }
            options={[
              ['A4', 'A4'],
              ['LEGAL', 'Legal'],
              ['80MM', 'Thermal receipt · 80 mm'],
            ]}
          />
        </Section>
        <Section
          title="Receipt wording"
          description="Keep official receipt text concise and consistent with school policy."
        >
          <TextField
            label="Receipt header"
            value={form.receiptHeaderText ?? ''}
            onChange={(value) => setValue('receiptHeaderText', value)}
          />
          <TextArea
            label="Receipt footer"
            value={form.receiptFooterText ?? ''}
            onChange={(value) => setValue('receiptFooterText', value)}
          />
        </Section>
        <Section
          title="Official document footers"
          description="These are used by eligible document generators. They do not publish a document by themselves."
        >
          <TextArea
            label="ID card footer"
            value={form.idCardFooterText ?? ''}
            onChange={(value) => setValue('idCardFooterText', value)}
          />
          <TextArea
            label="Payslip footer"
            value={form.payslipFooterText ?? ''}
            onChange={(value) => setValue('payslipFooterText', value)}
          />
          <TextArea
            label="Certificate footer"
            value={form.certificateFooterText ?? ''}
            onChange={(value) => setValue('certificateFooterText', value)}
          />
          <TextArea
            label="Report card footer"
            value={form.reportCardFooterText ?? ''}
            onChange={(value) => setValue('reportCardFooterText', value)}
          />
        </Section>
      </section>
      ...
      <p className="text-xs text-slate-500">
        Last updated:{' '}
        {branding.updatedAt
          ? formatBsDateTime(branding.updatedAt)
          : 'Not yet configured'}
      </p>
      {changed ? (
        <div className="fixed inset-x-0 bottom-0 z-20 border-t border-slate-200 bg-white/95 px-6 py-4 shadow-lg backdrop-blur">
          <div className="mx-auto flex max-w-5xl items-center justify-between gap-4">
            ...
            <div className="flex gap-2">
              <Button
                variant="outline"
                onClick={() => setForm(withoutFile(branding))}
              >
                Discard
              </Button>
      ...
      <ConfirmDialog
        ...
        description="This removes the current protected logo from school branding. Existing generated documents will not be rewritten."
...
function withoutFile(branding: BrandingDocumentsSettings): BrandingForm {
  const { logoFileAssetId: _file, updatedAt: _updated, ...form } = branding;
  return form;
}
function changedPayload(
  next: BrandingForm,
  previous: BrandingForm,
): UpdateBrandingDocumentsPayload {
  return Object.fromEntries(
    Object.entries(next).filter(
      ([key, value]) => value !== previous[key as keyof BrandingForm],
    ),
  ) as UpdateBrandingDocumentsPayload;
}
function Section({
  title,
  description,
