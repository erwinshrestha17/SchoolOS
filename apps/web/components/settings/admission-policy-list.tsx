...
export function AdmissionPolicyList() {
  ...
  const academicYearNames = new Map((academicYearsQuery.data ?? []).map((year) => [year.id, year.name]));
  const classNames = new Map((classesQuery.data ?? []).map((schoolClass) => [schoolClass.id, schoolClass.name]));
    ...
    <div className="space-y-5">
      {canManage ? (
        <div className="flex justify-end">
          <Button type="button" onClick={() => router.push('/dashboard/settings/admissions/new')}>
      ...
      <KpiGrid>
        <KpiCard title="Active Policies" value={summary?.activePolicies ?? 0} loading={policiesQuery.isLoading} tone="success" />
        <KpiCard title="Policies needing review" value={summary?.policiesNeedingReview ?? 0} loading={policiesQuery.isLoading} tone="warning" />
        <KpiCard title="Applications waiting for documents" value={summary?.applicationsWaitingForDocuments ?? 0} loading={policiesQuery.isLoading} tone="info" />
        <KpiCard title="Applications waiting for decision" value={summary?.applicationsWaitingForDecision ?? 0} loading={policiesQuery.isLoading} tone="neutral" />
      </KpiGrid>
      ...
      <DataTable<AdmissionPolicySummary>
        ...
        onRowClick={(policy) => router.push(`/dashboard/settings/admissions/${policy.id}`)}
        ...
        columns={[
          {
            header: 'Policy',
            cell: (policy) => (
              <Link href={`/dashboard/settings/admissions/${policy.id}`} className="font-bold text-slate-950 hover:underline" onClick={(event) => event.stopPropagation()}>
                {policy.name}
                {policy.isDefault ? <span className="ml-2 rounded-full bg-slate-100 px-2 py-0.5 text-[0.65rem] font-bold uppercase text-slate-500">Default</span> : null}
              </Link>
            ),
          },
          {
            ...
            cell: (policy) => (
              <span className="text-slate-600">
                {[
                  policy.classId ? classNames.get(policy.classId) ?? 'Class' : null,
                  policy.academicYearId ? academicYearNames.get(policy.academicYearId) ?? null : null,
                  policy.gradeBand ? policy.gradeBand.replace(/_/g, ' ') : null,
                  policy.applicantType !== 'BOTH' ? (policy.applicantType === 'TRANSFER' ? 'Transfer' : 'New admission') : null,
                ]
                  .filter(Boolean)
                  .join(', ') || 'All admissions'}
              </span>
