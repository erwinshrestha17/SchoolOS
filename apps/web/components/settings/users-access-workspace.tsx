...
export function UsersAccessWorkspace() {
  ...
  const createMutation = useMutation({
    ...
    onError: () =>
      setNotice({
        kind: 'error',
        text: 'Could not create the user. Check the account details and selected roles.',
      }),
  });
  const statusMutation = useMutation({
      ...
      setNotice({
        ...
        text:
          variables.status === 'SUSPENDED'
            ? 'User suspended and active sessions handled by the server.'
    ...
    onError: (error) =>
      setNotice({
        kind: 'error',
        text: getSchoolFriendlyError(
          error,
          'Could not update this user account status.',
        ),
      }),
  });
  const resetMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      setNotice({
        kind: 'success',
        text: 'Password reset. The user must change the temporary password at next sign in.',
      });
      await client.invalidateQueries({ queryKey: ['settings', 'users'] });
    },
    onError: () =>
      setNotice({
        kind: 'error',
        text: 'Could not reset this password. Check policy and permissions.',
      }),
  });
  ...
  if (usersQuery.isLoading || (canCreate && rolesQuery.isLoading))
    return (
  ...
  if (usersQuery.isError || (canCreate && rolesQuery.isError))
    return (
      <div className="p-6">
        <ErrorState
          title="Could not load users and access"
          message="Please retry to load school accounts and roles."
          error={usersQuery.error ?? rolesQuery.error}
          onRetry={() => {
            void usersQuery.refetch();
            if (canCreate) void rolesQuery.refetch();
          }}
        />
      </div>
    );
  ...
  const createUser = () => {
    ...
    if (
      ...
      setNotice({
        kind: 'error',
        text: 'Enter an email, a password of at least 8 characters, and at least one school role.',
      });
      return;
    }
  ...
  return (
    <div className="space-y-6 p-6 pb-24">
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <Stat label="Active accounts" value={counts.active} icon={UsersRound} />
        <Stat label="Pending accounts" value={counts.pending} icon={KeyRound} />
        <Stat
      ...
      <section
        className={`grid gap-5 ${canCreate ? 'xl:grid-cols-[0.85fr_1.15fr]' : ''}`}
      >
        {canCreate ? (
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="font-bold text-slate-950">Add school user</h2>
            <p className="mt-1 text-sm leading-6 text-slate-600">
              Create a staff or administrator account, then assign only the
              roles needed for school work.
            </p>
            <div className="mt-5 space-y-4">
              <Field
                ...
                placeholder="staff@school.edu.np"
                type="email"
              />
              <Field
                  ...
                  {roles.map((role) => {
                    ...
                    return (
                      <button
                        ...
                        className={`rounded-lg border px-3 py-2 text-sm font-semibold transition ${active ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-200 bg-white text-slate-700 hover:border-slate-400'}`}
                      >
                        {role.name}
                      </button>
                    );
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-slate-100 p-5">
            <div>
              <h2 className="font-bold text-slate-950">School accounts</h2>
              <p className="mt-1 text-sm text-slate-600">
                {users.length} accounts in this tenant.
              </p>
            </div>
          </div>
          {users.length === 0 ? (
            <div className="p-10 text-center">
              ...
              <p className="mt-3 font-semibold text-slate-900">
                No school user accounts
              </p>
              <p className="mt-1 text-sm text-slate-600">
                {canCreate
                  ? 'Create an account and assign a school role.'
                  : 'No accounts are available to review.'}
              </p>
            </div>
          ) : (
            <div className="divide-y divide-slate-100">
              {users.map((user) => (
                <div
                  key={user.id}
                  data-testid={`settings-user-row-${user.email ?? user.id}`}
      ...
      <ConfirmDialog
        ...
        destructive={pendingStatus?.status === 'SUSPENDED'}
        isConfirming={statusMutation.isPending}
        onConfirm={() => {
          if (pendingStatus)
            statusMutation.mutate({
              userId: pendingStatus.userId,
        ...
        {pendingStatus?.status === 'SUSPENDED' ? (
          <label className="block px-1">
            ...
            <span className="mt-0.5 block text-xs text-slate-500">
              Required when suspending a School Configuration Owner. Recorded in
              the audit log.
            </span>
            <input
              value={statusReason}
...
function getSchoolFriendlyError(error: unknown, fallback: string) {
  if (
...
function StatusBadge({ status }: { status: SchoolUserStatus }) {
  const className =
    status === 'ACTIVE'
...
function passwordPolicyIssue(password: string) {
  ...
  if (['admin123', 'password123', 'school123'].includes(password.toLowerCase()))
