...
export function UsersAccessWorkspace() {
  ...
  const statusMutation = useMutation({
    mutationFn: ({ userId, status, reason }: { userId: string; status: SchoolUserStatus; reason?: string }) => api.updateUserStatus(userId, { status, ...(reason?.trim() ? { reason: reason.trim() } : {}) }),
    ...
    onError: (error) => setNotice({ kind: 'error', text: getSchoolFriendlyError(error, 'Could not update this user account status.') }),
  });
  ...
  return <div className="space-y-6 p-6 pb-24">
    ...
    <ConfirmDialog isOpen={Boolean(pendingStatus)} title={pendingStatus?.status === 'SUSPENDED' ? 'Suspend user account' : 'Activate user account'} description={pendingStatus?.status === 'SUSPENDED' ? `Suspend ${pendingStatus.email ?? 'this account'}? The backend will revoke its active sessions. At least one active School Configuration Owner must always remain.` : `Activate ${pendingStatus?.email ?? 'this account'}?`} confirmLabel={pendingStatus?.status === 'SUSPENDED' ? 'Suspend account' : 'Activate account'} destructive={pendingStatus?.status === 'SUSPENDED'} isConfirming={statusMutation.isPending} onConfirm={() => { if (pendingStatus) statusMutation.mutate({ userId: pendingStatus.userId, status: pendingStatus.status, reason: statusReason }); }} onClose={() => { setPendingStatus(null); setStatusReason(''); }}>
      {pendingStatus?.status === 'SUSPENDED' ? <label className="block px-1"><span className="text-sm font-bold text-slate-900">Reason</span><span className="mt-0.5 block text-xs text-slate-500">Required when suspending a School Configuration Owner. Recorded in the audit log.</span><input value={statusReason} onChange={(event) => setStatusReason(event.target.value)} placeholder="Why is this account being suspended?" className="mt-2 h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none focus:border-slate-500 focus:ring-2 focus:ring-slate-900/5" /></label> : null}
    </ConfirmDialog>
