...
export function UsersAccessWorkspace() {
  ...
  return <div className="space-y-6 p-6 pb-24">
    ...
    <section className="grid gap-5 xl:grid-cols-[0.85fr_1.15fr]">
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="flex items-center justify-between border-b border-slate-100 p-5"><div><h2 className="font-bold text-slate-950">School accounts</h2><p className="mt-1 text-sm text-slate-600">{users.length} accounts in this tenant.</p></div></div>{users.length === 0 ? <div className="p-10 text-center"><UsersRound className="mx-auto h-7 w-7 text-slate-400" /><p className="mt-3 font-semibold text-slate-900">No school user accounts</p><p className="mt-1 text-sm text-slate-600">Create an account and assign a school role.</p></div> : <div className="divide-y divide-slate-100">{users.map((user) => <div key={user.id} className="flex flex-col gap-3 p-5 sm:flex-row sm:items-center"><div className="min-w-0 flex-1"><p className="truncate font-bold text-slate-950">{user.email ?? user.phone ?? 'User account'}</p><p className="mt-1 text-sm text-slate-600">{user.roles.map((role) => role.name).join(' · ') || 'No role assigned'}</p><p className="mt-1 text-xs font-semibold text-slate-500">{user.profileType} · {user.lastLoginAt ? 'Signed in before' : 'No recorded sign-in'}</p></div><div className="flex items-center justify-between gap-3 sm:justify-end"><StatusBadge status={user.status} /><Button type="button" size="sm" variant={user.status === 'SUSPENDED' ? 'outline' : 'outline'} onClick={() => setPendingStatus({ userId: user.id, status: user.status === 'SUSPENDED' ? 'ACTIVE' : 'SUSPENDED', email: user.email })}>{user.status === 'SUSPENDED' ? 'Activate' : 'Suspend'}</Button></div></div>)}</div>}</div>
    </section>
