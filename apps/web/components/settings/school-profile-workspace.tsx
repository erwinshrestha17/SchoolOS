...
const emptyProfile: ProfileForm = { schoolName: null, schoolAddress: null, schoolPhone: null, schoolEmail: null, schoolPanNumber: null, principalName: null, municipality: null, wardNumber: null, district: null, province: null, schoolType: null, iemisSchoolCode: null };
...
export function SchoolProfileWorkspace() {
  ...
  if (profileQuery.isLoading) return <div className="space-y-4 p-6"><div className="h-24 animate-pulse rounded-2xl bg-slate-100" /><div className="h-[520px] animate-pulse rounded-2xl bg-slate-100" /></div>;
  if (profileQuery.isError || !profileQuery.data) return <ErrorState error={profileQuery.error} onRetry={() => void profileQuery.refetch()} />;
  ...
  return <div className="space-y-6 p-6 pb-28">
    ...
    <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
      <Section title="Official identity" description="Legal school information. Changes are audited.">
        <TextField label="School name" value={form.schoolName ?? ''} onChange={(value) => setValue('schoolName', value)} required />
        <SelectField label="School type" value={form.schoolType ?? ''} onChange={(value) => setValue('schoolType', parseSchoolType(value))} options={[['', 'Select school type'], ['PRIVATE', 'Private / Institutional'], ['COMMUNITY', 'Community / Government'], ['TRUST', 'Public Trust']]} />
        <TextField label="PAN / registration number" value={form.schoolPanNumber ?? ''} onChange={(value) => setValue('schoolPanNumber', value)} />
        <TextField label="iEMIS school code" value={form.iemisSchoolCode ?? ''} onChange={(value) => setValue('iemisSchoolCode', value)} />
        <TextField label="Principal name" value={form.principalName ?? ''} onChange={(value) => setValue('principalName', value)} required />
      </Section>
      <Section title="Contact & location" description="Use the official contact information parents and staff should rely on.">
        <TextField label="School address" value={form.schoolAddress ?? ''} onChange={(value) => setValue('schoolAddress', value)} required className="md:col-span-2" />
        <TextField label="Contact phone" value={form.schoolPhone ?? ''} onChange={(value) => setValue('schoolPhone', value)} required />
        <TextField label="Contact email" type="email" value={form.schoolEmail ?? ''} onChange={(value) => setValue('schoolEmail', value)} required />
        <TextField label="Municipality" value={form.municipality ?? ''} onChange={(value) => setValue('municipality', value)} />
        <TextField label="Ward number" type="number" value={form.wardNumber?.toString() ?? ''} onChange={(value) => setValue('wardNumber', value ? Number(value) : null)} />
        <TextField label="District" value={form.district ?? ''} onChange={(value) => setValue('district', value)} />
        <TextField label="Province" value={form.province ?? ''} onChange={(value) => setValue('province', value)} />
      </Section>
    </section>
    <section className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm text-amber-950"><p className="font-bold">Additional school profile preferences are not editable yet</p><p className="mt-1 leading-6">Short name, language preference, office hours, offered school levels, and expanded principal or owner details need a tenant-scoped backend contract before they can be saved here. Timezone remains fixed to Asia/Kathmandu by the current SchoolOS Nepal operating standard.</p><p className="sr-only">needs API contract</p></section>
    <p className="text-xs text-slate-500">Last updated: {profile.updatedAt ? formatBsDateTime(profile.updatedAt) : 'Not yet configured'}</p>
    {changed ? <div className="fixed inset-x-0 bottom-0 z-20 border-t border-slate-200 bg-white/95 px-6 py-4 shadow-lg backdrop-blur"><div className="mx-auto flex max-w-5xl items-center justify-between gap-4"><p className="text-sm font-semibold text-slate-700">You have unsaved school profile changes.</p><div className="flex gap-2"><Button variant="outline" onClick={() => setForm(withoutUpdatedAt(profile))}>Discard</Button><Button onClick={save} disabled={updateMutation.isPending}><Save className="h-4 w-4" />{updateMutation.isPending ? 'Saving…' : 'Save changes'}</Button></div></div></div> : null}
  </div>;
}

function withoutUpdatedAt(profile: SchoolProfileSettings): ProfileForm { const { updatedAt: _updatedAt, ...form } = profile; return form; }
function changedPayload(next: ProfileForm, previous: ProfileForm): UpdateSchoolProfilePayload { return Object.fromEntries(Object.entries(next).filter(([key, value]) => value !== previous[key as keyof ProfileForm])) as UpdateSchoolProfilePayload; }
function parseSchoolType(value: string): ProfileForm['schoolType'] { return value === 'PRIVATE' || value === 'COMMUNITY' || value === 'TRUST' ? value : null; }
