          ...
          deliveries.slice(0, 12).map((delivery) => (
            ...
            >
              ...
              <div className="mt-3 grid gap-2 text-sm text-[var(--muted)]">
                ...
                <p className="text-xs">
                  {formatEnumLabel(delivery.sourceType)}
                </p>
              </div>
            </article>
          ))
    ...
    <div className="rounded-2xl border border-[var(--color-mod-notices-border)] bg-[var(--color-mod-notices-bg)] p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex items-start gap-3">
          <UsersRound
            className="mt-0.5 text-[var(--color-mod-notices-text)]"
            size={20}
          />
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-slate-600">
              Confirm resolved audience and channel counts before sending a
              high-impact notice.
            </p>
          </div>
        </div>
      ...
      {preview ? (
        <div className="mt-4 space-y-3">
          ...
          <p className="text-xs font-semibold text-slate-600">
            Channels:{' '}
            {preview.channels.map(formatEnumLabel).join(', ') || 'Unavailable'}
          </p>
          ...
          {isHighImpact ? (
            <label className="flex items-start gap-3 rounded-xl border border-rose-200 bg-white p-3 text-sm text-slate-700">
              ...
              />
              <span>
                I reviewed the recipient and channel counts for this high-impact
                notice.
              </span>
            </label>
          ) : null}
