...
export function LedgerSection({
  ...
  const sort: PaginatedDataTableSort | null =
    ledgerSortByParam &&
    (ledgerSortDirectionParam === 'asc' || ledgerSortDirectionParam === 'desc')
  ...
  const updateFilters = (updates: Record<string, string | number>) => {
    ...
    if (
      'ledgerSearch' in updates ||
      'ledgerStatus' in updates ||
      'ledgerSortBy' in updates
    ) {
  ...
  return (
    <div className="space-y-6">
      {mode === 'receipts' ? (
        <nav
          aria-label="Receipt center sections"
          className="flex flex-wrap gap-2 rounded-xl border border-slate-200 bg-white p-2"
        >
          <a
            href="#receipt-history"
            className="inline-flex min-h-10 items-center rounded-lg px-3 text-sm font-semibold text-slate-700 hover:bg-slate-50"
          >
            Receipt history
          </a>
          <a
      ...
      {mode !== 'invoices' ? (
        <div id="verify-receipt">
          <ReceiptVerificationPanel />
        </div>
      ) : null}

      {mode !== 'receipts' ? (
        <FilterBar label="Invoice filters">
                      ...
                      {receiptsQuery.data.items.map((receipt) => (
                        <tr key={receipt.id} className="hover:bg-slate-50">
                          ...
                          <td className="px-4 py-3.5 text-slate-700">
                            {receipt.student?.name ?? 'Student unavailable'}
                          </td>
                          <td className="px-4 py-3.5 text-slate-600">
                            {receipt.invoiceNumber ?? 'Unavailable'}
                          </td>
                          ...
                          <td className="px-4 py-3.5 text-slate-600">
                            {receipt.method ?? 'Unavailable'}
                          </td>
                          <td className="px-4 py-3.5 text-slate-600">
                            {formatBsDateTime(receipt.issuedAt)}
                          </td>
                          <td className="px-4 py-3.5">
                            <StatusBadge status={receipt.fileStatus} />
                          </td>
                          <td className="px-4 py-3.5">
