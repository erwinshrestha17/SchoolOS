        ...
        <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div>
            <h3 className="text-2xl font-black text-slate-900 flex items-center gap-3">
              ...
              {entry?.entryNumber ? `Entry ${entry.entryNumber}` : 'Loading entry'}
            </h3>
            <p className="text-xs font-black uppercase tracking-[0.2em] text-slate-400 mt-2 ml-[3.25rem]">
              {entry ? `Recorded on ${new Date(entry.entryDate).toLocaleDateString()}` : 'Fetching ledger details'}
            </p>
          </div>
          <button 
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="bg-[var(--color-mod-accounting-bg)] p-6 rounded-2xl text-sm border border-[var(--color-mod-accounting-border)] space-y-2">
                <p className="flex items-start gap-2">
                  <span className="font-black text-[var(--color-mod-accounting-text)] uppercase tracking-widest text-[10px] mt-0.5">Narration</span>
                  <span className="font-bold text-[var(--color-mod-accounting-text)] leading-relaxed italic">&ldquo;{entry.narration}&rdquo;</span>
                </p>
                <p className="flex items-center gap-2 pt-2 border-t border-[var(--color-mod-accounting-border)]">
                  <span className="font-black text-[var(--color-mod-accounting-text)] uppercase tracking-widest text-[10px]">Source</span>
                  <span className="font-bold text-[var(--color-mod-accounting-text)]">{entry.sourceType} <span className="text-[10px] opacity-70">#{entry.sourceId || 'Source ID not set'}</span></span>
                </p>
              </div>
                    ...
                    {entry.lines?.map((line: any) => (
                      <tr key={line.id} className="hover:bg-slate-50/30 transition-colors group">
                        <td className="px-6 py-4">
                          <p className="font-bold text-slate-900 group-hover:text-[var(--color-mod-accounting-text)] transition-colors">{line.chartAccount.name}</p>
