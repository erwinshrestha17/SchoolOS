  ...
  const submitMapping = () => {
    ...
    try {
      ...
      setFormError(
        error instanceof Error
          ? error.message
          : 'Enter valid BS effective dates.',
      );
    }
  };
      ...
      },
      {
        ...
        cell: (item) => (
          <div className="text-xs text-slate-600">
            ...
            <p className="mt-1">
              to{' '}
              {item.effectiveTo ? formatBsDate(item.effectiveTo) : 'open-ended'}
            </p>
          </div>
        ),
      },
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-black text-slate-950">
                Source posting mappings
              </h2>
              <p className="mt-1 text-sm text-slate-600">
                Effective-dated debit and credit configuration for approved
                SchoolOS source modules.
              </p>
            </div>
          </div>
        </div>
      ...
      {healthQuery.isLoading ? (
        ...
        >
          <div className="grid gap-4 md:grid-cols-3">
            <HealthMetric
              label="Sampled posted entries"
              value={healthQuery.data.sampledPostedSourceEntries}
              tone="neutral"
            />
            <HealthMetric
              label="Missing source references"
              value={healthQuery.data.missingSourceId.count}
              tone={
                healthQuery.data.missingSourceId.count > 0
                  ? 'danger'
                  : 'success'
              }
            />
            <HealthMetric
              ...
              value={
                healthQuery.data.isClean ? 'No sampled issues' : 'Needs review'
              }
              tone={healthQuery.data.isClean ? 'success' : 'warning'}
            />
          </div>
            ...
            {healthQuery.data.modules.map((module) => (
              ...
              >
                ...
                <p className="mt-1 text-xs text-slate-500">
                  {module.postedCount} sampled posting(s) ·{' '}
                  {module.missingSourceIdCount} missing source reference(s)
                </p>
              </div>
            ))}
          </div>
          <p className="mt-5 text-xs leading-5 text-slate-500">
            This health check is bounded and does not prove end-to-end posting
        ...
        <div className="grid gap-3 border-b border-slate-100 pb-5 md:grid-cols-[minmax(0,1fr)_180px_180px]">
          <FormField label="Search mappings">
            <Input
              value={search}
              onChange={(event) =>
                updateFilters({ search: event.target.value, page: undefined })
              }
        ...
        >
          <div className="my-8 w-full max-w-3xl rounded-2xl bg-white p-6 shadow-xl">
            ...
            <p className="mt-2 text-sm text-slate-600">
              Create a reviewed, effective-dated mapping. Overlapping date
              ranges are rejected.
            </p>
            {accountsQuery.isError ? (
              <PageState
                tone="danger"
                title="Chart accounts could not be loaded"
                actionLabel="Retry"
              ...
              <div className="mt-6 grid gap-4 md:grid-cols-2">
                <FormField label="Source module">
                  <Select
                    ...
                    onChange={(event) =>
                      setForm((current) => ({
                        ...current,
                        sourceModule: event.target.value as SourceModule,
                      }))
                    }
                  >
                    {sourceModules.map((module) => (
                ...
                <FormField label="Source event / type">
                  <Input
                ...
                <FormField label="Posting type">
                  <Input
                    ...
                    onChange={(event) =>
                      setForm((current) => ({
                        ...current,
                        postingType: event.target.value,
                      }))
                    }
                ...
                <BsDateField
                  ...
                  onChange={(value) =>
                    setForm((current) => ({
                      ...current,
                      effectiveFromBs: value,
                    }))
                  }
                  required
                />
                <BsDateField
                  label="Effective to (BS, optional)"
                  value={form.effectiveToBs}
                  onChange={(value) =>
                    setForm((current) => ({ ...current, effectiveToBs: value }))
                  }
                />
              </div>
            )}
      ...
      {archiveTarget ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="archive-mapping-title"
        >
          <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3
                  id="archive-mapping-title"
                  className="text-lg font-black text-slate-950"
                >
                  Archive source mapping?
                </h3>
                <p className="mt-2 text-sm text-slate-600">
                  This closes the active mapping version. Posted journals remain
                  unchanged.
                </p>
              </div>
            </div>
            <FormField label="Reason" className="mt-5">
              <Input
                value={archiveReason}
                onChange={(event) => setArchiveReason(event.target.value)}
                placeholder="Why is this mapping being archived?"
              />
            </FormField>
            {archiveMutation.isError ? (
              ...
              >
                {archiveMutation.error instanceof Error
                  ...
                  : 'The mapping could not be archived.'}
              </p>
            ) : null}
            <div className="mt-6 flex justify-end gap-3">
              <Button
...
function AccountCell({
  ...
  account: AccountingSourceMappingSummary['debitAccount'];
}) {
  return (
    <>
...
function invalidateSourceMappingQueries(
