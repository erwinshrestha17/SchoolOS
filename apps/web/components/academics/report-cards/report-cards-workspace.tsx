...
export function ReportCardsWorkspace() {
  ...
  const [selectedReportCardId, setSelectedReportCardId] = useState<
    string | null
  >(null);
  const [correctionTargetId, setCorrectionTargetId] = useState<string | null>(
    null,
  );
  const [correctionReason, setCorrectionReason] = useState('');
  const [generationStudentIds, setGenerationStudentIds] = useState<
    string[] | null
  >(null);
      ...
      cell: (row: any) => (
        <div className="flex flex-col">
          <span className="font-bold text-slate-900">
            {row.student?.firstNameEn} {row.student?.lastNameEn}
          </span>
      ...
      cell: (row: any) => (
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={() => setSelectedReportCardId(row.id)}
          >
          ...
          {row.status === 'LOCKED' && (
            <Button
              variant="ghost"
              size="sm"
              onClick={() => prepareCorrection(row.id)}
        ...
        <TabsContent value="generation">
          <div className="space-y-8">
              ...
              <StatCard
                ...
                value={
                  reportsQuery.data?.length
                    ? (
                        (reportsQuery.data as any[]).reduce(
                          (acc: number, r: any) => acc + Number(r.gpa),
                          0,
                        ) / reportsQuery.data.length
                      ).toFixed(2)
                    : '0.00'
            ...
            <FilterBar
              ...
              actions={
                <Button
                  ...
                  disabled={
                    !classId || !examTermId || generateMutation.isPending
                  }
                  className="border-none bg-[var(--color-mod-academics-accent)] shadow-sm hover:bg-[var(--color-mod-academics-text)]"
                >
            ...
            >
              ...
              <Select
                value={examTermId}
                onChange={(e: any) => setExamTermId(e.target.value)}
                className="lg:w-48"
              >
                ...
                {!!examTermId && !!classId ? (
                  ...
                  <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 bg-white px-6 py-20 text-center shadow-sm">
                    ...
                    <p className="mt-2 text-sm font-medium text-slate-400">
                      Generation controls will appear once context is selected.
                    </p>
                  </div>
                )}
              </div>
                      ...
                      {(gradingPolicyQuery.data?.scale ?? []).some(
                        ...
                        <div className="flex items-start gap-2 rounded-2xl border border-amber-100 bg-amber-50 p-3 text-xs font-semibold text-amber-800">
                          ...
                          <p>
                            ...
                            Review school settings before generating locked
                            report cards.
                          </p>
                        </div>
                      )}
                ...
                >
                  <div className="space-y-4">
                    ...
                    <div className="rounded-2xl bg-amber-50 border border-amber-100 p-4 text-xs text-amber-800 font-medium">
                      Generating report cards will automatically lock marks for
                ...
                >
                      ...
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                          <p className="text-sm font-black text-amber-900">
                            Correction review open
                          </p>
                          <p className="text-xs font-medium leading-5 text-amber-800">
                            A locked report-card correction must be reviewed in
                            the dialog before regeneration.
                          </p>
                        </div>
                        <Button
                          type="button"
                          ...
                          variant="outline"
                          onClick={() =>
                            setCorrectionTargetId(correctionTargetId)
                          }
                        >
                          Review correction
                        </Button>
                  ...
                  {selectedReportCardId ? (
                    <div
                      ...
                      data-testid="report-card-history"
                    >
                      {(historyQuery.data?.history ?? []).map((item: any) => (
                        ...
                        >
                          ...
                          <div className="text-xs text-slate-500">
                            Grade {item.grade} / GPA{' '}
                            {Number(item.gpa).toFixed(2)}
                          </div>
                        </div>
                      ))}
                      {(historyQuery.data?.corrections ?? []).map(
                        (item: any) => (
                          <div
                            key={item.id}
                      ...
                      {!historyQuery.isLoading &&
                        ...
                        (historyQuery.data?.corrections ?? []).length === 0 && (
          ...
          >
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="mt-2 text-sm text-slate-500 max-w-sm mb-6">
                All advanced academic reports have been moved to the central
                Reporting & Exports module for a consistent experience.
              </p>
              <Link
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="rounded-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Regeneration creates a new report-card version and keeps the
            original artifact in history.
          </DialogDescription>
        </DialogHeader>
        ...
        >
          <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
            ...
            <div className="mt-3 grid gap-3 text-sm sm:grid-cols-2">
              <CorrectionMeta
                label="Student"
                value={reportCardStudentName(reportCard)}
              />
              <CorrectionMeta
                label="Version"
                value={
                  reportCard?.version ? `v${reportCard.version}` : 'Latest'
                }
              />
              <CorrectionMeta
                label="Grade"
                value={reportCard?.grade ?? 'Not set'}
              />
              <CorrectionMeta
                label="GPA"
                value={
                  reportCard?.gpa !== undefined && reportCard?.gpa !== null
                    ? Number(reportCard.gpa).toFixed(2)
                    : 'Not set'
                }
              />
              <CorrectionMeta
                label="Status"
                value={
                  [reportCard?.status, reportCard?.publishStatus]
                    .filter(Boolean)
                    .join(' / ') || 'Locked'
                }
              />
          ...
          <div className="rounded-2xl border border-amber-100 bg-amber-50 p-4 text-xs font-semibold leading-5 text-amber-800">
            Use this only after confirming the marks source has been corrected.
            The previous locked report card remains auditable and the
            regenerated card becomes a new version.
          </div>
