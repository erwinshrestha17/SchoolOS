...
export function MarksEntryTab({
  ...
  const [queuedMarksMessage, setQueuedMarksMessage] = useState<string | null>(
    null,
  );
  ...
  const componentsQuery = useQuery({
    ...
    queryFn: () =>
      api.listComponentsByExamTerm(filters.examTermId, {
  ...
  const selectedComponent = componentsQuery.data?.find(
    (c: any) => c.id === filters.assessmentComponentId,
  );
  const maxMarks = selectedComponent ? Number(selectedComponent.maxMarks) : 100;
  const passMarks = selectedComponent
  ...
  const changedEntries = useMemo(() => {
    ...
    return Array.from(studentIds).map((studentId) => ({
      ...
      isAbsent:
        statuses[studentId] === 'ABSENT' || statuses[studentId] === 'EXCUSED',
    ...
    <div className="space-y-10 animate-fade-in">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-5 items-end">
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              onChange={(e) => {
                const value = e.target.value;
                requestFilterChange(() =>
                  setFilters((c) => ({ ...c, classId: value, sectionId: '' })),
                );
              }}
              className="premium-input bg-white"
            >
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              onChange={(e) => {
                const value = e.target.value;
                requestFilterChange(() =>
                  setFilters((c) => ({ ...c, sectionId: value })),
                );
              }}
              className="premium-input bg-white"
            >
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              onChange={(e) => {
                const value = e.target.value;
                requestFilterChange(() =>
                  setFilters((c) => ({ ...c, assessmentComponentId: value })),
                );
              }}
              className="premium-input bg-white"
            >
              <option value="">Select Component</option>
              {componentsQuery.data?.map((c: any) => (
                <option key={c.id} value={c.id}>
                  {c.name} ({c.maxMarks})
                </option>
              ))}
          ...
          <div className="flex gap-2">
            <Button
              ...
              onClick={() =>
                queryClient.invalidateQueries({ queryKey: ['marks', filters] })
              }
            >
              <Search size={20} />
            </Button>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest mt-0.5">
                Successfully saved {saveSuccess} entries.
              </p>
            </div>
          </div>
          <Button
        ...
        <div className="p-6 bg-amber-50 border border-amber-100 rounded-2xl flex items-center gap-4 text-amber-800 animate-in fade-in duration-500">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold opacity-80 uppercase tracking-widest mt-0.5">
              Contact the academic coordinator to request an unlock for this
              exam term.
            </p>
          </div>
        </div>
      ...
      {!isLocked && lockedStudentIds.size > 0 && (
        <div className="p-6 bg-amber-50 border border-amber-100 rounded-2xl flex items-center gap-4 text-amber-800 animate-in fade-in duration-500">
          ...
          <div>
            <p className="text-sm font-black tracking-tight uppercase">
              {lockedStudentIds.size} student
              {lockedStudentIds.size === 1 ? '' : 's'} still locked
            </p>
            <p className="text-[10px] font-bold opacity-80 uppercase tracking-widest mt-0.5">
              Marked students were finalized while the term was locked and need
              an approved correction request before they can be edited again.
            </p>
          </div>
        </div>
      )}
      ...
      {filters.assessmentComponentId ? (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50 border-b border-slate-100">
                  ...
                  <th className="py-6 px-6 font-black uppercase tracking-widest text-[10px] text-slate-400 w-32 text-center">
                    Status
                  </th>
                  <th className="py-6 px-6 font-black uppercase tracking-widest text-[10px] text-slate-400 w-32 text-center">
                    Existing
                  </th>
                  <th className="py-6 px-6 font-black uppercase tracking-widest text-[10px] text-slate-400 w-48">
                    Score Entry
                  </th>
                  <th className="py-6 px-6 font-black uppercase tracking-widest text-[10px] text-slate-400">
                    Observations
                  </th>
                  <th className="py-6 px-6 font-black uppercase tracking-widest text-[10px] text-slate-400 w-20 text-right">
                    Actions
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {existingMarksQuery.isError || rosterQuery.isError ? (
                  ...
                  studentsForClass.map((student, index) => {
                    ...
                    const currentStatus =
                      statuses[student.id] || existing?.status || 'SUBMITTED';
                    const numericValue =
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-4 px-6 text-center text-xs font-black text-slate-700 tracking-tighter">
                          {student.rollNumber ?? '—'}
                        </td>
                        <td className="py-4 px-6">
                            ...
                            <input
                              ...
                              onKeyDown={(e) => {
                                if (
                                  ...
                                  e.preventDefault();
                                  document
                                    .getElementById(`mark-input-${index + 1}`)
                                    ?.focus();
                                }
                                if (e.key === 'ArrowUp') {
                                  e.preventDefault();
          ...
          <div className="flex items-center justify-between border-t border-slate-100 bg-slate-50/70 p-4">
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">
                  Total Roster: {studentsForClass.length}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <div className="h-2 w-2 rounded-full bg-emerald-500" />
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">
                  Draft Entries: {changedEntries.length}
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2 rounded-xl border border-[var(--color-mod-academics-border)] bg-white px-3 py-1.5 text-[10px] font-black uppercase tracking-widest text-[var(--color-mod-academics-text)]">
                <Zap
                  size={14}
                  className="text-[var(--color-mod-academics-accent)]"
                />
        ...
        <section className="rounded-2xl border-2 border-dashed border-slate-200 bg-white p-16 text-center">
          ...
          <p className="mt-2 text-sm font-bold text-slate-400 max-w-sm mx-auto leading-relaxed">
            Select context to begin. Choose an exam term, class, and assessment
            component above to load the mark entry grid.
          </p>
          <div className="mt-8 flex items-center justify-center gap-3">
            <div className="px-4 py-2 rounded-2xl bg-slate-100 text-[10px] font-black uppercase tracking-widest text-slate-500">
              Fast Entry
            </div>
            <div className="px-4 py-2 rounded-2xl bg-slate-100 text-[10px] font-black uppercase tracking-widest text-slate-500">
              Auto-Sync
