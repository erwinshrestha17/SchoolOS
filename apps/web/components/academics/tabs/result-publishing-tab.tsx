...
export function ResultPublishingTab({
  ...
  const sectionsForClass = useMemo(
    () => allSections.filter((s: any) => s.classId === selection.classId),
    [allSections, selection.classId],
  );
  ...
  const handleBatchUnpublish = () => {
    if (selectedIds.size === 0) return;
    const unpublishable = records.filter(
      (r) => selectedIds.has(r.reportCardId) && r.publishStatus === 'PUBLISHED',
    );
  ...
  const confirmBatchUnpublish = () => {
    ...
    unpublishMut.mutate({
      reportCardIds: unpublishable.map((r) => r.reportCardId),
    });
  };
  ...
  return (
    <div className="space-y-10 animate-fade-in">
      ...
      {selection.examTermId ? (
          ...
          <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mt-1">
                  Control visibility to {records.length} students
                </p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div className="relative w-64">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSelection((f) => ({ ...f, search: e.target.value }))
                  }
                          ...
                          <div className="flex flex-col items-end gap-1">
                            ...
                            {r.publishedAt && (
                              <span className="text-[7px] font-bold text-slate-300 uppercase tracking-widest">
                                Released {formatBsDate(r.publishedAt)}
                              </span>
                            )}
                          </div>
                        </td>
            ...
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-[var(--color-mod-academics-accent)]">
                  Total Published:{' '}
                  {
                    records.filter((r) => r.publishStatus === 'PUBLISHED')
                      .length
                  }
                </span>
              </div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">
                  Awaiting Lock:{' '}
                  {records.filter((r) => r.reportStatus !== 'LOCKED').length}
                </span>
              </div>
            </div>
        ...
        <section className="rounded-2xl border-2 border-dashed border-slate-200 bg-white p-16 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm font-bold text-slate-400 max-w-sm mx-auto leading-relaxed">
            Select an exam term to control result visibility and notify
      ...
      <ConfirmDialog
        ...
        isConfirming={
          publishMut.isPending || notifyMut.isPending || unpublishMut.isPending
        }
        onConfirm={
          confirmAction === 'publish'
            ? confirmBatchPublish
