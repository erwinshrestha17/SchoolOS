...
export function PromotionTab({ academicYears, classes, allSections }: Props) {
  ...
  const [detailStudent, setDetailStudent] = useState<PromotionReadiness | null>(
    null,
  );
  ...
  const fromSections = useMemo(
    () => allSections.filter((s: any) => s.classId === promo.fromClassId),
    [allSections, promo.fromClassId],
  );
  const toSectionsForClass = useMemo(
    () => allSections.filter((s: any) => s.classId === promo.toClassId),
    [allSections, promo.toClassId],
  );
  ...
  const handleBatchPromote = () => {
    ...
    const readyCount = students.filter(
      (s) => selectedIds.has(s.studentId) && s.status === 'READY',
    ).length;
  ...
  return (
    <div className="space-y-10 animate-fade-in">
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between mb-8">
          <div>
            ...
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mt-1">
              Batch Student Lifecycle Transition
            </p>
          </div>
          <Button
            type="button"
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          ...
          <div className="space-y-6 p-8 rounded-2xl bg-[var(--color-mod-academics-surface)] border border-[var(--color-mod-academics-border)] shadow-sm">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-[var(--color-mod-academics-accent)]">
                Destination Target
              </span>
            </div>
      ...
      {promo.fromClassId ? (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50 border-b border-slate-100">
                  ...
                  <th className="py-4 px-8 font-black uppercase tracking-widest text-[10px] text-slate-400 text-right">
                    Checks
                  </th>
                </tr>
              </thead>
                      ...
                      <tr
                        ...
                        className={cn(
                          ...
                          selectedIds.has(s.studentId) &&
                            'bg-[var(--color-mod-academics-surface)]',
                        )}
                      >
                          ...
                          {s.outstandingBalance > 0 ? (
                            <div className="inline-flex flex-col items-center">
                              <span className="text-xs font-black text-rose-600 tracking-tight">
                                Rs {s.outstandingBalance.toLocaleString()}
                              </span>
                        ...
                        <td className="py-4 px-6">
                          <div className="flex flex-col gap-1">
                            ...
                            />
                            {s.reasons.length > 0 && (
                              <p className="text-[8px] font-bold text-slate-400 uppercase tracking-tight leading-none ml-1">
                                {s.reasons[0]}
                              </p>
                            )}
                          </div>
                        ...
                        <td className="py-4 px-8 text-right">
                          <div className="flex items-center justify-end gap-2">
                            {!isReady && (
                              <div
                                className="h-8 w-8 rounded-xl bg-slate-50 text-slate-300 flex items-center justify-center"
        ...
        <section className="rounded-2xl border-2 border-dashed border-slate-200 bg-white p-16 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm font-bold text-slate-400 max-w-sm mx-auto leading-relaxed">
            Select a class above to analyze student eligibility for promotion to
      ...
      <Dialog
        ...
        onOpenChange={(open: boolean) => !open && setDetailStudent(null)}
      >
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>{detailStudent?.studentName}</DialogTitle>
            <DialogDescription>
              {detailStudent?.studentSystemId} — Grade {detailStudent?.grade},
              GPA {detailStudent?.gpa.toFixed(2)}
            </DialogDescription>
          </DialogHeader>
