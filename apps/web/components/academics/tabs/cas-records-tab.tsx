...
const casTemplates = [
  {
    category: 'Classwork',
    maxScore: 20,
    note: 'Classwork completion and effort',
  },
  {
    category: 'Homework',
    maxScore: 10,
    note: 'Homework quality and submission consistency',
  },
  {
    category: 'Project',
    maxScore: 25,
    note: 'Project work, creativity, and presentation',
  },
  {
    category: 'Presentation',
    maxScore: 15,
    note: 'Speaking confidence and clarity',
  },
  {
    category: 'Lab Work',
    maxScore: 20,
    note: 'Practical/lab participation and accuracy',
  },
  {
    category: 'Participation',
    maxScore: 10,
    note: 'Class participation and engagement',
  },
  {
    category: 'Discipline',
    maxScore: 10,
    note: 'Discipline, cooperation, and classroom behavior',
  },
  {
    category: 'Creative Work',
    maxScore: 20,
    note: 'Creative task and expression',
  },
  {
    category: 'Social Skills',
    maxScore: 10,
    note: 'Peer collaboration and social development',
  },
...
export function CasRecordsTab({
  ...
  const [cas, setCas] = useState<CasFormState>(() =>
    makeDefaultForm(academicYears),
  );
  ...
  const [deleteTarget, setDeleteTarget] = useState<CasRecordSummary | null>(
    null,
  );
  ...
  const subjectsForClass = useMemo(
    ...
    [
      assignmentScope.assignedSubjectIds,
      assignmentScope.isScoped,
      subjects,
      cas.classId,
    ],
  ...
  const formRosterQuery = useQuery({
    ...
    queryFn: () =>
      api.listStudents({
        classId: cas.classId,
        sectionId: cas.sectionId || undefined,
        limit: 1000,
      }),
  ...
  const filterStudentsForClass = (filterRosterQuery.data?.items ??
    []) as StudentRosterRow[];
  ...
  const invalidSingleScore =
    cas.score < 0 || cas.score > cas.maxScore || cas.maxScore <= 0;
  ...
  const invalidBatchEntries = batchEntries.filter(
    (entry) => entry.score < 0 || entry.score > cas.maxScore,
  );
  ...
  const handleKeyDown = (
    ...
    if (key === 'ArrowDown' || (key === 'Enter' && !shiftKey)) {
      ...
      key === 'ArrowRight' &&
      (e.currentTarget as any).selectionEnd ===
        (e.currentTarget as any).value?.length
    ) {
      nextCol++;
    } else if (
      key === 'ArrowLeft' &&
      (e.currentTarget as any).selectionStart === 0
    ) {
      nextCol--;
    } else if (key === 'Escape') {
        ...
        <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between mb-8">
          <div>
            ...
            <p className="mt-1 text-sm font-semibold text-slate-500">
              Behavioral and formative observations.
            </p>
          </div>
          {successMessage && (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
          <div className="space-y-2">
            ...
            <select
              ...
              onChange={(e) =>
                setFilters((c) => ({ ...c, academicYearId: e.target.value }))
              }
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              onChange={(e) =>
                setFilters((c) => ({ ...c, subjectId: e.target.value }))
              }
              className="premium-input bg-white"
            >
              <option value="">All Subjects</option>
              {filterSubjectsForClass.map((subject) => (
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              onChange={(e) =>
                setFilters((c) => ({ ...c, studentId: e.target.value }))
              }
              className="premium-input bg-white"
            >
              <option value="">All Students</option>
              {filterStudentsForClass.map((student) => (
          ...
          <section className="group relative rounded-2xl border border-slate-200 bg-white p-8 transition-all hover:border-[var(--color-mod-academics-border)] hover:shadow-sm">
            <div className="mb-8">
              ...
              <h3 className="text-base font-bold text-slate-900">
                {editingId ? 'Modify Record' : 'Single Entry'}
              </h3>
            ...
            <div className="space-y-4">
              ...
              <select
                value={cas.studentId}
                onChange={(e) =>
                  setCas((c) => ({ ...c, studentId: e.target.value }))
                }
                className="premium-input bg-slate-50"
              >
              ...
              <select
                data-testid="cas-entry-subject"
                value={cas.subjectId}
                onChange={(e) =>
                  setCas((c) => ({ ...c, subjectId: e.target.value }))
                }
                className="premium-input bg-slate-50"
              >
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  className="premium-input border-[var(--color-mod-academics-border)] bg-[var(--color-mod-academics-surface)] text-[var(--color-mod-academics-text)] shadow-sm"
                >
                  {casTemplates.map((template) => (
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    value={cas.score}
                    onChange={(e) =>
                      setCas((c) => ({ ...c, score: Number(e.target.value) }))
                    }
                    className="premium-input bg-slate-50 font-black tracking-tighter"
                  />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">
              ...
              <textarea
                value={cas.note}
                onChange={(e) =>
                  setCas((c) => ({ ...c, note: e.target.value }))
                }
          ...
          <section className="rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-sm">
            <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Multi-student entry for {cas.category}.
                </p>
              </div>
              <Button
                type="button"
              ...
              >
                ...
                {batchMutation.isPending
                  ? 'Syncing'
                  : `Sync ${batchEntries.length} Records`}
              </Button>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50/50 border-b border-slate-100">
                    <th className="py-4 px-8 font-black uppercase tracking-widest text-[10px] text-slate-400">
                      Student Info
                    </th>
                    <th className="py-4 px-6 font-black uppercase tracking-widest text-[10px] text-slate-400 w-32 text-center">
                      Score / {cas.maxScore}
                    </th>
                    <th className="py-4 px-6 font-black uppercase tracking-widest text-[10px] text-slate-400">
                      Notes
                    </th>
                  </tr>
                </thead>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-sm">
        <div className="p-8 border-b border-slate-100 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Audit trail of recorded observations.
            </p>
          </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                ...
                <th className="py-4 px-6 font-black uppercase tracking-widest text-[10px] text-slate-400 text-center">
                  Outcome
                </th>
                <th className="py-4 px-6 font-black uppercase tracking-widest text-[10px] text-slate-400">
                  Timeline
                </th>
                <th className="py-4 px-6 font-black uppercase tracking-widest text-[10px] text-slate-400 text-right">
                  Actions
                </th>
              </tr>
            </thead>
                  ...
                  <td className="py-4 px-8">
                    <div className="flex flex-col">
                      <span className="text-sm font-black text-slate-900 uppercase tracking-tight">
                        {record.student?.firstNameEn}{' '}
                        {record.student?.lastNameEn}
                      </span>
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                        {record.class?.name} / {record.section?.name ?? 'Main'}
                      </span>
                    </div>
                  </td>
                  <td className="py-4 px-6">
                    <div className="flex items-center gap-2">
                      <span className="px-2 py-1 rounded-lg bg-slate-100 text-[10px] font-black text-slate-600 uppercase tracking-widest">
                        {record.subject?.code}
                      </span>
                      <span className="px-2 py-1 rounded-lg bg-teal-50 text-[10px] font-black text-teal-600 uppercase tracking-widest border border-teal-100">
                        {record.category}
                      </span>
                    </div>
                  </td>
                    ...
                    <div className="inline-flex flex-col items-center px-4 py-2 rounded-2xl bg-slate-50 border border-slate-100 group-hover:bg-white transition-colors">
                      ...
                      <div className="w-12 h-1 bg-slate-100 rounded-full mt-1 overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${(Number(record.score) / Number(record.maxScore)) * 100}%`,
                          }}
                        />
                      </div>
                    </div>
