...
export function AdmissionEntry({
  initialMode,
  initialCaseId,
}: {
  initialMode?: 'direct' | 'review';
  initialCaseId?: string;
}) {
  const { hasPermissions } = useSession();
  const [mode, setMode] = useState<'choose' | 'direct' | 'review'>(
    initialMode ?? 'choose',
  );
  ...
  if (policyQuery.isLoading && !initialMode) {
    return (
      <div className="flex min-h-48 items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white text-sm font-semibold text-slate-600">
        ...
        Checking this school’s admission policy…
      </div>
    );
  }
  ...
  if (policyQuery.isError) {
    return (
      <ErrorState
        ...
        message="No student has been created. Retry before starting a new admission."
  ...
  if (mode === 'direct')
    return <AdmissionCaseWizard initialCaseId={initialCaseId} />;
  if (mode === 'review') return <AdmissionReviewCaseForm />;

  const schoolDefaultPolicy = policyQuery.data?.policies.find(
    (policy) => policy.isDefault,
  );
  const directIsDefault =
    schoolDefaultPolicy?.admissionMode === 'DIRECT_ALLOWED';

  return (
    <section>
      <div className="mb-5">
        <h2 className="text-xl font-black text-slate-950">
          Choose the admission path
        </h2>
        <p className="mt-1 text-sm text-slate-600">
          Start with the path that matches the school-office task. School policy
          makes the final admission decision.
        </p>
      </div>
      <div className="grid gap-4 md:grid-cols-2">
        <article
          className={`rounded-2xl border bg-white p-6 shadow-sm ${directIsDefault ? 'border-blue-300 ring-2 ring-blue-100' : 'border-slate-200'}`}
        >
          ...
          <p className="mt-2 text-sm leading-6 text-slate-600">
            For a parent currently at the front desk.
          </p>
          <Button
            className="mt-6"
            type="button"
        ...
        <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-600">
            Find and continue an application that was already started.
          </p>
          <Link
            href="/dashboard/admissions"
            className="mt-6 inline-flex h-11 items-center justify-center rounded-2xl border-2 border-slate-200 bg-white px-6 text-sm font-bold text-slate-700 transition hover:border-slate-300 hover:bg-slate-50"
          >
            Open admission queue
          </Link>
        </article>
        <article
          className={`rounded-2xl border bg-white p-6 shadow-sm ${!directIsDefault ? 'border-violet-300 ring-2 ring-violet-100' : 'border-slate-200'}`}
        >
          ...
          <p className="mt-2 text-sm leading-6 text-slate-600">
            For transfers, exceptions, or admissions needing approval.
          </p>
          <Button
            className="mt-6"
            type="button"
        ...
        <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-600">
            Validate and review a school-prepared admission CSV before creating
            records.
          </p>
          <Link
