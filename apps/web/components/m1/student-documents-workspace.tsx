...
export function StudentDocumentsWorkspace() {
  ...
  const [
    guardianRemovalEvidenceReference,
    setGuardianRemovalEvidenceReference,
  ] = useState('');
  const [guardianRemovalReplacementId, setGuardianRemovalReplacementId] =
    useState('');
  const [guardianRemovalAccessReviewed, setGuardianRemovalAccessReviewed] =
    useState(false);
  ...
  const [uploadMetadataError, setUploadMetadataError] = useState<string | null>(
    null,
  );
  const searchParams = useSearchParams();
  const requestedStudentId =
    searchParams.get('studentId') ?? searchParams.get('student');
  ...
  useEffect(() => {
    if (!requestedStudentId || selectedStudent?.id === requestedStudentId)
      return;
    const requestedStudent = studentsQuery.data?.items.find(
  ...
  function buildUploadMetadata() {
    ...
    if (selectedDocument && trimmedReason.length < 5) {
      throw new Error(
        'Enter a replacement reason before uploading this document.',
      );
    }

    let expiryDate: string | null = null;
    if (uploadExpiryDateBs.trim()) {
      const gregorian = toGregorianDateFromBs(
        parseBsDateInput(uploadExpiryDateBs),
      );
  ...
  const uploadMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedStudent)
        throw new Error('Select a student before uploading.');
    ...
    onSuccess: () => {
      setToast({
        ...
        description:
          'The file is pending school review and is not complete until verified.',
      });
      setUploadReason('');
      setUploadNotes('');
  ...
  const verifyMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setToast({
        ...
        description: 'The document review and audit details were recorded.',
      });
      void queryClient.invalidateQueries({
        queryKey: ['student-profile', selectedStudent?.id],
      });
  ...
  const archiveMutation = useMutation({
    ...
    onSuccess: () => {
      setToast({
        ...
        description:
          'The document was archived and the audit reason was recorded.',
      });
    ...
    onError: (error) =>
      setToast({
        ...
        description: schoolFacingErrorMessage(error, {
          ...
          invalid: 'Record the required archive reason before continuing.',
          forbidden:
  ...
  const removeGuardianMutation = useMutation({
    mutationFn: () => {
      if (!selectedStudent || !guardianToRemove)
        throw new Error('Select a guardian link to remove.');
      if (!guardianRemovalAccessReviewed)
        throw new Error(
          'Confirm guardian portal and protected-file access review before revoking access.',
        );
      if (guardianToRemove.isPrimary && !guardianRemovalReplacementId)
        throw new Error(
          'Choose another primary guardian before removing this one.',
        );
      return api.removeStudentGuardianAccess(
        selectedStudent.id,
        guardianToRemove.id,
        {
          ...
          newPrimaryGuardianId: guardianToRemove.isPrimary
            ? guardianRemovalReplacementId
            : null,
        },
      );
    },
    onSuccess: () => {
      setToast({
        ...
        description:
          'The relationship was preserved as revoked history and the protected-file access review was recorded.',
      });
  ...
  if (studentsQuery.isLoading && !requestedStudentId)
    return <LoadingState variant="page" label="Loading student documents…" />;
  if (studentsQuery.isError && !requestedStudentId) {
    return (
      <DocumentWorkspaceFailure
  ...
  const expiring = documents.filter(
    (document) =>
      document.expiryDate &&
      new Date(document.expiryDate).getTime() < Date.now() + 30 * 86400000,
  ).length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-5 xl:grid-cols-[240px_minmax(0,1fr)] 2xl:grid-cols-[260px_minmax(0,1fr)_300px]">
        <aside className="h-fit rounded-2xl border border-slate-200 bg-white p-4 shadow-sm xl:sticky xl:top-24">
          ...
          <div className="mt-3 max-h-[620px] space-y-1 overflow-y-auto">
            ...
            {studentsQuery.isError ? (
              <p className="rounded-xl border border-warning-100 bg-warning-50 p-3 text-xs font-semibold text-warning-800">
                Student list is unavailable. The linked student record can still
                load from the protected profile route.
              </p>
            ) : null}
            {(studentsQuery.data?.items ?? []).map((student) => {
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-center gap-3 rounded-xl p-2 text-left transition ${selectedStudent?.id === student.id ? 'bg-primary-50 ring-1 ring-primary-200' : 'hover:bg-slate-50'}`}
                >
        ...
        <main className="min-w-0 space-y-5">
          {!selectedStudent ? (
            requestedStudentId && requestedProfileQuery.isError ? (
              <DocumentWorkspaceFailure
                error={requestedProfileQuery.error}
                onRetry={() => void requestedProfileQuery.refetch()}
              />
            ) : requestedStudentId ? (
              <LoadingState
                variant="skeleton"
                label="Loading linked student documents…"
              />
            ) : (
              <EmptyState
                title="Select a student"
                description="Choose a student to review guardians, document checklist, protected files, and verification history."
              />
            )
          ) : profileQuery.isLoading ? (
            <LoadingState
              variant="skeleton"
              label="Loading protected student record…"
            />
            ...
            <>
              <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                  <div className="flex items-center gap-4">
                    <Avatar
                      initials={(
                        profileQuery.data.student.fullNameEn ?? 'ST'
                      ).slice(0, 2)}
                      size="lg"
                    />
                    <div>
                      <div className="flex items-center gap-2">
                        <h2 className="text-lg font-black text-slate-950">
                          {profileQuery.data.student.fullNameEn}
                        </h2>
                        <StatusBadge
                          status={
                            profileQuery.data.student.lifecycleStatus ??
                            'ACTIVE'
                          }
                        />
                      </div>
                      <p className="mt-1 text-xs font-semibold text-slate-500">
                        {profileQuery.data.student.studentSystemId} ·{' '}
                        {profileQuery.data.student.className ?? 'No class'} /{' '}
                        {profileQuery.data.student.sectionName ?? 'No section'}
                      </p>
                      {selectedDocument ? (
                        <p className="mt-2 rounded-xl border border-warning-100 bg-warning-50 px-3 py-2 text-xs font-semibold text-warning-800">
                          Uploading now will create a replacement record for{' '}
                          {selectedDocument.fileName}. The existing record is
                  ...
                  <div className="grid w-full gap-3 md:max-w-xl">
                    <div className="grid gap-3 sm:grid-cols-2">
                      ...
                      <BsDateField
                        ...
                        value={uploadExpiryDateBs}
                        onChange={(value) => {
                          setUploadExpiryDateBs(value);
                          setUploadMetadataError(null);
                        }}
                        error={
                          uploadMetadataError?.toLowerCase().includes('date')
                            ? uploadMetadataError
                            : null
                        }
                    ...
                    <label className="block text-sm font-semibold text-slate-700">
                      ...
                      <input
                        ...
                        placeholder={
                          selectedDocument
                            ? 'Why is this document being replaced?'
                            : 'Optional upload reason'
                        }
                        className="mt-1.5 h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-900 outline-none focus:border-slate-400 focus:ring-2 focus:ring-slate-900/5"
                      />
                    </label>
                    <label className="block text-sm font-semibold text-slate-700">
                      ...
                      <textarea
                        ...
                        placeholder="Optional note for school review"
                        className="mt-1.5 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-slate-400 focus:ring-2 focus:ring-slate-900/5"
                      />
                    </label>
                    {uploadMetadataError &&
                    !uploadMetadataError.toLowerCase().includes('date') ? (
              ...
              <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div className="border-b border-slate-100 p-4">
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    Protected files linked to this student through File
                    Registry.
                  </p>
                </div>
                {documents.length === 0 ? (
                  <EmptyState
                    ...
                    description="Upload the first protected document for this student."
                  />
                ) : (
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {documents.map((document) => (
                          <tr
                            ...
                            className={`cursor-pointer ${selectedDocument?.id === document.id ? 'bg-primary-50' : 'hover:bg-slate-50'}`}
                          >
                            ...
                            <td className="px-4 py-3 text-slate-600">
                              {document.expiryDate
                                ? formatBsDate(document.expiryDate)
                                : '—'}
                            </td>
        ...
        <aside
          className="h-fit rounded-2xl border border-slate-200 bg-white p-5 shadow-sm xl:col-span-2 2xl:col-span-1 2xl:sticky 2xl:top-24"
        ...
        >
          ...
          <div className="border-b border-slate-100 py-4">
            <div className="flex items-center justify-between">
              ...
              <StatusBadge
                status={`${expiryTemplatesQuery.data?.filter((item) => item.isActive).length ?? 0} ACTIVE`}
                tone="info"
              />
            </div>
            {expiryTemplatesQuery.isError ? (
              <p className="mt-2 text-xs font-bold text-danger-700">
                Expiry policies could not load.
              </p>
            ) : (expiryTemplatesQuery.data?.length ?? 0) === 0 ? (
              <p className="mt-2 text-xs text-slate-500">
                No document-expiry reminder templates configured.
              </p>
            ) : (
              <div className="mt-2 space-y-2">
                {expiryTemplatesQuery.data?.slice(0, 4).map((template) => (
                  <div
                    key={template.id}
                    className="rounded-lg bg-slate-50 p-2 text-xs"
                  >
                    <p className="font-bold text-slate-800">
                      {template.reminderStatus} · {template.channel}
                    </p>
                    <p className="mt-0.5 text-slate-500">
                      {template.daysBeforeExpiry
                        ? `${template.daysBeforeExpiry} days before expiry`
                        : 'At expiry'}{' '}
          ...
          {!selectedDocument ? (
            <p className="py-10 text-center text-sm text-slate-500">
              Select a document to view protected file controls, metadata,
            ...
            <div className="space-y-4 pt-4">
              <div>
                <p className="text-xs font-black uppercase tracking-wide text-slate-500">
                  {selectedDocument.kind.replace(/_/g, ' ')}
                </p>
                <p className="mt-1 break-all text-sm font-bold text-slate-900">
                  {selectedDocument.fileName}
                </p>
              </div>
              <dl className="space-y-2 rounded-xl bg-slate-50 p-3 text-xs">
                <div className="flex justify-between gap-3">
                  <dt className="text-slate-500">Status</dt>
                  <dd>
                    <StatusBadge
                      status={formatDocumentStatus(selectedDocument.status)}
                    />
                  </dd>
                </div>
                <div className="flex justify-between gap-3">
                  ...
                  <dd className="font-bold">
                    {Math.ceil(selectedDocument.sizeBytes / 1024)} KB
                  </dd>
                </div>
                <div className="flex justify-between gap-3">
                  <dt className="text-slate-500">Uploaded</dt>
                  <dd className="font-bold">
                    {formatBsDateTime(selectedDocument.uploadedAt)}
                  </dd>
                </div>
              </dl>
              {selectedDocument.status === 'REJECTED' &&
              selectedDocument.notes ? (
                <p className="rounded-xl border border-danger-100 bg-danger-50 p-3 text-xs font-semibold text-danger-700">
                  Reviewer note: {latestReviewNote(selectedDocument.notes)}
                </p>
              ) : null}
              <label className="block text-xs font-bold text-slate-700">
                ...
                <textarea
                  ...
                  placeholder="Record review notes. Required when rejecting."
                  className="mt-2 w-full rounded-xl border border-slate-200 p-3 font-normal"
                />
              </label>
              <div className="grid gap-2">
                <StudentDocumentAccessButton
                  studentId={selectedDocument.studentId}
                ...
                <Button
                  ...
                  onClick={() =>
                    verifyMutation.mutate({
                      ...
                      notes:
                        reviewReason.trim() ||
                        'Verified from M1 document workspace',
                    })
                  }
                ...
                <Button
                  ...
                  disabled={
                    verifyMutation.isPending || reviewReason.trim().length < 5
                  }
                >
                  Reject document
                </Button>
                ...
                >
                  <Archive className="h-4 w-4" /> Archive document
                </Button>
              </div>
              <div className="border-t border-slate-100 pt-4">
                ...
                {historyQuery.isError ? (
                  <p className="mt-3 rounded-lg border border-warning-100 bg-warning-50 p-2 text-xs font-semibold text-warning-800">
                    Document history could not load. The protected file and
                    admission record were not changed.
                  </p>
                ) : (
                  <div className="mt-3 space-y-3">
                    {(historyQuery.data ?? [])
                      ...
                      .map((entry) => (
                        <div
                          key={entry.id}
                          className="border-l-2 border-primary-200 pl-3 text-xs"
                        >
                          <p className="font-bold text-slate-800">
      ...
      <ConfirmDialog
        ...
        confirmDisabled={
          guardianRemovalReason.trim().length < 5 ||
          guardianRemovalEvidenceReference.trim().length < 3 ||
          !guardianRemovalAccessReviewed ||
          Boolean(guardianToRemove?.isPrimary && !guardianRemovalReplacementId)
        }
        onClose={() => {
          setGuardianToRemove(null);
          setGuardianRemovalReason('');
      ...
      >
        {guardianToRemove?.isPrimary ? (
          <label className="mb-4 block text-sm font-bold text-slate-700">
            ...
            <select
              value={guardianRemovalReplacementId}
              onChange={(event) =>
                setGuardianRemovalReplacementId(event.target.value)
              }
              className="mt-2 h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-900"
            >
              <option value="">Select replacement primary</option>
              {(profileQuery.data?.guardians ?? [])
                .filter((guardian) => guardian.id !== guardianToRemove.id)
                .map((guardian) => (
                  <option key={guardian.id} value={guardian.id}>
                    {guardian.fullName} · {guardian.relation}
                  </option>
                ))}
            </select>
          </label>
        ...
        <label className="block text-sm font-bold text-slate-700">
          ...
          <textarea
            rows={3}
            value={guardianRemovalReason}
            onChange={(event) => setGuardianRemovalReason(event.target.value)}
            placeholder="Why should this guardian relationship and access be removed?"
        ...
        <label className="mt-4 block text-sm font-bold text-slate-700">
          Evidence reference
          <input
            value={guardianRemovalEvidenceReference}
            onChange={(event) =>
              setGuardianRemovalEvidenceReference(event.target.value)
            }
            placeholder="Safe case or register reference"
            className="mt-2 font-normal"
          />
        </label>
        <label className="mt-4 flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs font-bold text-slate-700">
          <input
            type="checkbox"
            checked={guardianRemovalAccessReviewed}
            onChange={(event) =>
              setGuardianRemovalAccessReviewed(event.target.checked)
            }
            className="mt-0.5 h-4 w-4 shrink-0"
          />
          I reviewed guardian portal and protected-file access before revoking
          this relationship.
        </label>
      </ConfirmDialog>
      <ConfirmDialog
