import type {
  ...
  UpdateSchoolSettingsDomainPayload,
  UpdateSchoolSettingsDomainResult,
} from '@schoolos/core';
import {
  readFileAsBase64,
  request,
...
export const schoolSettingsApi = {
  ...
  getSchoolIntegrationsStatus: () =>
    request<SchoolIntegrationsStatus>(`${basePath}/integrations`),
  getSchoolProfile: () =>
  ...
  getBrandingDocuments: () =>
    request<BrandingDocumentsSettings>(`${basePath}/branding-documents`),
  updateBrandingDocuments: (body: UpdateBrandingDocumentsPayload) =>
    request<BrandingDocumentsSettings>(`${basePath}/branding-documents`, {
      method: 'PATCH',
      json: body,
    }),
  updateSchoolSettingsDomain: (
    domain: SchoolSettingsDomain,
    body: UpdateSchoolSettingsDomainPayload,
  ) =>
    request<UpdateSchoolSettingsDomainResult>(`/settings/domains/${domain}`, {
      method: 'PATCH',
      json: body,
    }),
  uploadSchoolLogo: async (file: File, note?: string) =>
    request<TenantLogoUploadResult>('/settings/branding/logo', {
      method: 'POST',
      json: {
        fileName: file.name,
        mimeType: file.type,
