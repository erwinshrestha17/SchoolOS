...
export type PayrollExceptionParams = {
  search?: string;
  year?: number;
  month?: number;
  payrollRunId?: string;
...
export const payrollApi = {
  listStaffOptions: (
    ...
    request<RemoteLookupPage<StaffLookupOption>>(
      withQuery("/staff/options", params),
      { signal },
    ),
  ...
  listStaffDocuments: (
    ...
    request<any>(
      withQuery(
        `/hr/staff/${encodeURIComponent(staffId)}/documents`,
        params ?? {},
      ),
    ),
  ...
  exportPayrollRunRegisterCsv: (id: string) =>
    downloadCsv(
      ...
      `payroll-register-${id}-${getNepalSchoolDay().gregorianDate}.csv`,
    ),
