...
export const studentsApi = {
  ...
  getIemisReadiness: (studentId: string) => request<StudentIemisReadiness>(`/students/${encodeURIComponent(studentId)}/iemis-readiness`),
  ...
  previewDuplicateStudentMerge: (body: { sourceStudentId: string; targetStudentId: string }) =>
    request<{
      sourceStudent: {
        id: string;
  ...
  mergeDuplicateStudent: (body: { sourceStudentId: string; targetStudentId: string; reason: string }) =>
  ...
  upsertDocumentExpiryTemplate: (body: { channel: 'email' | 'sms'; reminderStatus: 'expired' | 'expiring'; subjectTemplate?: string; messageTemplate: string; daysBeforeExpiry?: number; isActive?: boolean }) =>
  ...
  openStudentDocumentPdf: async (studentId: string, kind: string) => {
    ...
    await openProtectedFile(artifact.fileAssetId, {
      fileName: artifact.fileName,
    });
  },
  listMyLinkedStudents: () => request<{ items: MyLinkedStudent[] }>('/mobile/me/students'),
};
