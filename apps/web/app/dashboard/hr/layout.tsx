...
import { 
  Users, LayoutDashboard, CalendarDays, 
    ...
    <div className="space-y-8 animate-fade-in">
      <header className="relative overflow-hidden rounded-[2rem] bg-slate-900 px-6 py-10 text-white shadow-2xl lg:px-12">
        ...
        <div className="relative flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-2xl">
            <div className="flex items-center gap-2 mb-4">
              <Badge variant="phase2" className="bg-blue-500/20 text-blue-400 border-blue-500/20">
                Human Resources
              </Badge>
            ...
            <h1 className="text-3xl font-extrabold tracking-tight sm:text-5xl">
              HR <span className="text-blue-400">Workspace</span>
            </h1>
            <p className="mt-4 text-lg text-slate-300 leading-relaxed">
              Manage staff profiles, contracts, leave workflows, and track attendance for <span className="font-bold text-white">{session?.tenant.name}</span>.
            </p>
          </div>
        ...
        <nav className="relative mt-10 flex flex-wrap gap-2 border-t border-white/10 pt-6">
          {navItems.map((item) => {
            const active = pathname === item.href || (item.href !== '/dashboard/hr' && pathname?.startsWith(item.href));
            return (
              <Link
                key={item.href}
                href={item.href}
