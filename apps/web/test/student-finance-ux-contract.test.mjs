...
describe("Student profile to finance collection UX contract", () => {
  ...
  it("keeps attendance and documents inside student-profile context", () => {
    ...
    assert.doesNotMatch(attendance, /studentSystemId/);
    assert.match(attendance, /getStudentAttendanceMonthlyRegister/);
    assert.match(attendance, /MONTH_PARAM/);
    assert.match(overview, /Review missing details/);
    assert.match(documents, /Upload document/);
    assert.match(documents, /Replace/);
    assert.match(documents, /Student ID Card/);
    assert.match(documents, /Transfer Certificate/);
    assert.match(documents, /Character Certificate/);
    assert.match(documents, /Official checklist totals are not included/);
    assert.doesNotMatch(
      documents,
      /Required student documents are verified and current/,
    );
  });
