...
describe('global topbar search (students + invoices)', () => {
  ...
  it('keeps student search real-API-backed and permission-gated', () => {
    ...
    assert.match(
      search,
      /canSearchStudents = hasPermissions\(\[['"]students:read['"]\]\)/,
    );
  });
  ...
  it('gates invoice search behind the same permission the backend route requires', () => {
    ...
    assert.match(
      search,
      /canSearchInvoices = hasPermissions\(\[['"]payments:collect['"]\]\)/,
    );
    assert.match(
      search,
      /api\.listInvoices\(\{ search: debouncedQuery, limit: 5 \}\)/,
    );
  });
  ...
  it('routes invoice results through the canonical fees collection contract', () => {
    assert.match(
      search,
      /\/dashboard\/fees\/collect\?\$\{params\.toString\(\)\}/,
    );
    assert.match(
      search,
  ...
  it('navigates a combined, keyboard-accessible result list', () => {
    assert.match(
      search,
      /results\.length\) % results\.length|current \+ 1\) % results\.length/,
    );
