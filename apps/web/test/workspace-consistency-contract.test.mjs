...
describe('SchoolOS workspace consistency contract', () => {
  it('composes every priority route from the canonical shell and header', () => {
    for (const route of Object.keys(routeSources)) {
      ...
      assert.match(
        source,
        /DashboardPageShell/,
        `${route} must use DashboardPageShell`,
      );
  ...
  it('keeps remaining modules on shared primitives with no duplicate KPI or tab implementations', () => {
    for (const route of remainingMigrationRoutes) {
      ...
      assert.doesNotMatch(
        source,
        /\b(?:KpiCard|KpiGrid|StatCard|ModuleTabs)\b/,
      );
      assert.doesNotMatch(
        source,
        /function\s+SummaryCard|const\s+SummaryCard\s*=/,
  ...
  it('retains task-specific operational workspaces', () => {
    ...
    for (const [route, marker] of Object.entries(markers)) {
      assert.match(
        sourceFor(route),
        marker,
        `${route} lost its operational purpose`,
      );
    }
  });
  ...
  it('keeps shared states and protected-file helpers in the migrated system', () => {
    const fixture = read(
      'components/test-fixtures/workspace-state-fixture.tsx',
    );
    for (const state of [
      'LoadingState',
      'EmptyState',
    ...
    assert.doesNotMatch(
      protectedSurfaces,
      /window\.open\([^)]*(?:file|attachment|payslip)/i,
    );
  });
  ...
  it('keeps M12 delivery, M15 notice authoring, and removed chat boundaries distinct', () => {
    ...
    assert.match(
      removedChat,
      /New conversations and messages are unavailable/i,
    );
    assert.doesNotMatch(
      sidebar,
      /href:\s*["']\/dashboard\/(?:messages|messaging)["']/,
    );
  });
