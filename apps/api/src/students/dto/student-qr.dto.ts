...
export class ResolveStudentQrDto {
  @ApiProperty({ example: 'scanned-token', description: 'The raw token from the QR code' })
  @IsString()
  @IsNotEmpty()
  token!: string;
...
export class RotateStudentQrDto {
  @ApiProperty({ required: false, description: 'Reason for rotating the QR credential' })
  @IsString()
  @IsOptional()
  reason?: string;
}
...
export class RevokeStudentQrDto {
  @ApiProperty({ required: false, description: 'Reason for revoking the QR credential' })
  @IsString()
  @IsOptional()
  reason?: string;
}
