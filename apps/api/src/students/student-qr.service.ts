...
export class StudentQrService {
   ...
   */
  async rotateQr(tenantId: string, studentId: string, actorUserId: string, reason?: string) {
    const existing = await this.prisma.studentQrCredential.findUnique({
      where: { tenantId_studentId: { tenantId, studentId } },
    });
   ...
   */
  async revokeQr(tenantId: string, studentId: string, actorUserId: string, reason?: string) {
    const existing = await this.prisma.studentQrCredential.findUnique({
      where: { tenantId_studentId: { tenantId, studentId } },
    });
   ...
   */
  async resolveQr(tenantId: string, token: string, purpose: StudentQrResolvePurpose, actorUserId: string) {
    const tokenHash = hashToken(token);
    ...
    if (!credential || credential.tenantId !== tenantId || credential.status !== StudentQrStatus.ACTIVE) {
      await this.auditService.record({
        ...
        after: { 
          purpose, 
          reason: !credential ? 'not_found' : credential.tenantId !== tenantId ? 'wrong_tenant' : 'revoked' 
        },
      });
