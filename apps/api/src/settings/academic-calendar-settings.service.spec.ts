...
describe('AcademicCalendarSettingsService', () => {
  ...
  function buildService() {
    ...
    const prisma = {
      academicYear: { findMany: jest.fn(), findFirst: jest.fn() },
    ...
    return {
      service: new AcademicCalendarSettingsService(
        prisma as never,
  ...
  it('stores a BS calendar day at Nepal local midnight in UTC', async () => {
    ...
    prisma.schoolCalendarDay.upsert.mockImplementation(
    ...
    prisma.academicYear.findFirst.mockResolvedValue({
      startsOn: new Date('2024-04-12T18:15:00.000Z'),
      endsOn: new Date('2024-04-13T18:15:00.000Z'),
    });

    await service.upsertCalendarDay(
      tenantId,
      {
    ...
    expect(input.where.tenantId_calendarDate.calendarDate.toISOString()).toBe(
      '2024-04-12T18:15:00.000Z',
    );
  ...
  it('creates BS academic years atomically and makes the requested year current', async () => {
    ...
    tx.academicYear.create.mockImplementation(
