...
import { AutosaveAdmissionDraftDto, EnhancedDuplicateReviewDto, GenerateTransferCertificateDto, GraduateStudentDto, IemisReadinessSummaryDto, ImportReviewQueueDto, RecoverAdmissionDraftsDto, RemoveStudentGuardianAccessDto, ResolveAdmissionRelationshipsDto } from './dto/m1-admissions-hardening.dto';
...
export class M1AdmissionsHardeningController {
  ...
  getOwnershipAudit(@Param('id') studentId: string, @CurrentAuth() auth: AuthContext) {
  ...
  autosaveAdmissionDraft(@Body() dto: AutosaveAdmissionDraftDto, @CurrentAuth() auth: AuthContext) {
    return this.service.autosaveAdmissionDraft(dto, auth);
  }

  @Get('drafts/recover')
  @Permissions('enrollments:read', 'students:read', 'guardians:read')
  recoverAdmissionDrafts(@Query() query: RecoverAdmissionDraftsDto, @CurrentAuth() auth: AuthContext) {
  ...
  resolveRelationships(@Body() dto: ResolveAdmissionRelationshipsDto, @CurrentAuth() auth: AuthContext) {
  ...
  removeGuardianAccess(@Param('studentId') studentId: string, @Param('guardianId') guardianId: string, @Body() dto: RemoveStudentGuardianAccessDto, @CurrentAuth() auth: AuthContext) {
  ...
  generateIdCard(@Param('id') studentId: string, @CurrentAuth() auth: AuthContext) {
  ...
  generateTransferCertificate(@Param('id') studentId: string, @Body() dto: GenerateTransferCertificateDto, @CurrentAuth() auth: AuthContext) {
  ...
  graduateStudent(@Param('id') studentId: string, @Body() dto: GraduateStudentDto, @CurrentAuth() auth: AuthContext) {
    return this.service.graduateStudent(studentId, dto, auth);
  }

  @Get('import-review/queue')
  @Permissions('enrollments:read', 'students:read')
  listImportReviewQueue(@Query() query: ImportReviewQueueDto, @CurrentAuth() auth: AuthContext) {
    return this.service.listImportReviewQueue(query, auth);
  }

  @Get('iemis/readiness-summary')
  @Permissions('students:read')
  getIemisReadinessSummary(@Query() query: IemisReadinessSummaryDto, @CurrentAuth() auth: AuthContext) {
