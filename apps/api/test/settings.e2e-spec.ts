...
describe('SchoolOS Tenant Settings (E2E)', () => {
  ...
  it('enforces tenant scoping and role-based permissions', async () => {
    ...
    prisma.__state.tenants.push({ id: tenantAId, slug: 'school-a', name: 'School A', isActive: true, plan: 'standard' });
    ensureTenantDefaults(prisma.__state, tenantAId);
    
    const adminRoleA = prisma.__state.roles.find(r => r.tenantId === tenantAId && r.name === 'admin');
    const adminA = { id: 'admin-a', email: 'admin@school-a.com', passwordHash: password, tenantId: tenantAId, status: 'ACTIVE' };
    prisma.__state.users.push(adminA);
    prisma.__state.userRoles.push({ userId: adminA.id, roleId: adminRoleA.id, tenantId: tenantAId });

    // 2. Setup Tenant B
    const tenantBId = 'tenant-b';
    prisma.__state.tenants.push({ id: tenantBId, slug: 'school-b', name: 'School B', isActive: true, plan: 'standard' });
    ...
    await settingsController.updateSetting('branding_primary_color', { value: '#ff0000' }, { auth: authA } as any);
    
    const settingsA = await settingsController.getSettings({ auth: authA } as any);
    expect(settingsA.find(s => s.key === 'branding_primary_color')?.value).toBe('#ff0000');

    // 5. Verify it's NOT in Tenant B
    const settingsB = prisma.__state.tenantSettings.filter(s => s.tenantId === tenantBId);
    expect(settingsB).toHaveLength(0);

    // 6. Test Validation - Invalid Color
    await expect(settingsController.updateSetting('branding_primary_color', { value: 'not-a-color' }, { auth: authA } as any))
      .rejects.toBeInstanceOf(BadRequestException);

    // 7. Test Validation - Invalid Type (number instead of string)
    await expect(settingsController.updateSetting('timezone', { value: 123 }, { auth: authA } as any))
      .rejects.toBeInstanceOf(BadRequestException);

    // 8. Test Permissions - Teacher (Read only)
    const teacherRoleA = prisma.__state.roles.find(r => r.tenantId === tenantAId && r.name === 'teacher');
    const teacherA = { id: 'teacher-a', email: 'teacher@school-a.com', passwordHash: password, tenantId: tenantAId, status: 'ACTIVE' };
    prisma.__state.users.push(teacherA);
    prisma.__state.userRoles.push({ userId: teacherA.id, roleId: teacherRoleA.id, tenantId: tenantAId });

    const authTeacher = { tenantId: tenantAId, userId: teacherA.id, roles: ['teacher'], permissions: SYSTEM_ROLE_PERMISSIONS['teacher'] };
...
async function createPrismaMock() {
  ...
  return {
    ...
    tenantSetting: {
      findMany: jest.fn(async (q) => state.tenantSettings.filter(s => s.tenantId === q.where.tenantId)),
      findUnique: jest.fn(async (q) => state.tenantSettings.find(s => s.tenantId === q.where.tenantId_key.tenantId && s.key === q.where.tenantId_key.key)),
      upsert: jest.fn(async (q) => {
        const existing = state.tenantSettings.find(s => s.tenantId === q.where.tenantId_key.tenantId && s.key === q.where.tenantId_key.key);
        if (existing) {
          Object.assign(existing, q.update, { updatedAt: new Date() });
          return existing;
        }
        const created = { id: `s-${state.tenantSettings.length}`, ...q.create, createdAt: new Date(), updatedAt: new Date() };
    ...
    user: {
      findUnique: jest.fn(async (q) => {
        const u = state.users.find(u => (u.tenantId === q.where?.tenantId_email?.tenantId && u.email === q.where?.tenantId_email?.email) || u.id === q.where.id);
        if (!u) return null;
        return {
          ...u,
          userRoles: state.userRoles.filter(ur => ur.userId === u.id).map(ur => ({
            ...ur,
            role: {
...
function ensureTenantDefaults(state: any, tenantId: string) {
  for (const def of SYSTEM_ROLE_DEFINITIONS) {
    const role = { id: `role-${tenantId}-${def.name}`, tenantId, name: def.name, isSystem: true };
