...
import { TransportTripStatus, TransportStudentTripStatus } from '@prisma/client';
...
describe('Transport Hardening (E2E)', () => {
  ...
  const adminActor: AuthContext = {
    ...
    userId: 'admin-1',
    roles: ['admin'],
    permissions: ['transport:manage', 'transport:routes:read', 'transport:trips:update', 'transport:trips:read', 'transport:location:update'],
  };
  ...
  describe('Driver Boundaries', () => {
    it('filters routes for drivers to only show assigned ones', async () => {
      ...
      const route1 = {
        ...
        name: 'Route 1',
        isActive: true,
        driverAssignments: [{ staff: { userId: 'driver-1' } }],
      };
      const route2 = {
        id: 'route-2',
        tenantId,
        name: 'Route 2',
        isActive: true,
        driverAssignments: [{ staff: { userId: 'other' } }],
      };
    ...
    it('prevents driver from updating a trip they are not assigned to', async () => {
      ...
      await expect(
        transportController.markStudentBoarded('trip-1', { studentId: 'stu-1' }, driverActor)
  ...
  describe('Trip Lifecycle Integrity', () => {
    it('enforces boarding before dropping', async () => {
      ...
      await expect(
        transportController.markStudentDropped('trip-1', { studentId: 'stu-1' }, driverActor)
      ).rejects.toThrow(ConflictException);

      // Board first
      await transportController.markStudentBoarded('trip-1', { studentId: 'stu-1' }, driverActor);
      
      // Now can drop
      prisma.__state.transportTripStudentStatuses[0].status = TransportStudentTripStatus.BOARDED;
      await expect(
        transportController.markStudentDropped('trip-1', { studentId: 'stu-1' }, driverActor)
    ...
    it('prevents completion if students are still boarded without FORCE_COMPLETE', async () => {
      ...
      await expect(
        transportController.completeTrip('trip-1', { notes: 'FORCE_COMPLETE' }, driverActor)
      ).resolves.toBeDefined();
    });
  });
  ...
  describe('GPS Ingestion Validation', () => {
    it('rejects invalid coordinates', async () => {
      const trip = { id: 'trip-1', tenantId, status: TransportTripStatus.ACTIVE, driverAssignmentId: 'assign-1' };
      prisma.__state.transportTrips = [trip];
      prisma.__state.transportDriverAssignments = [{ id: 'assign-1', tenantId, staff: { userId: 'driver-1' } }];

      await expect(
        transportController.recordLocationPing('trip-1', { latitude: 100, longitude: 50 }, driverActor)
  ...
  describe('Parent Boundaries', () => {
    ...
    it('allows parent to track their own student', async () => {
      const student = { id: 'stu-1', tenantId, firstNameEn: 'Sita', lastNameEn: 'Student' };
      prisma.__state.students = [student];
      prisma.__state.studentGuardians = [
        { tenantId, studentId: 'stu-1', guardian: { userId: 'parent-1' }, student }
      ];
      
      // Also need a trip for it to return something interesting
      const route = { id: 'r-1', name: 'Route 1', code: 'R1' };
      const vehicle = { id: 'v-1', registrationNumber: 'BA 1 PA 1234', model: 'Bus' };
      const driverAssignment = { id: 'd-1', staff: { id: 'st-1', firstName: 'D', lastName: 'R', user: { phone: '123' } } };
      const trip = { 
        id: 'trip-1', 
        tenantId, 
        status: TransportTripStatus.ACTIVE, 
      ...
      prisma.__state.transportTripStudentStatuses = [
        { id: 'ts-1', tenantId, studentId: 'stu-1', tripId: 'trip-1', status: 'PENDING', stopId: 'stop-1', trip, stop }
      ];
      ...
      const result = await transportController.getParentStudentActiveTrip('stu-1', parentActor);
