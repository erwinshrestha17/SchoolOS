...
| FEE-15 | M3 | 03 | PARTIAL | Refunds create immutable negative allocation history, restore each affected invoice, and expose M11 journal lineage. The register now carries the versioned metadata envelope, and its counts and money totals come from database aggregates over the whole filtered set instead of the fetched rows. Paging is a bounded merge: the two sources (refunds and reversed payments) cannot be offset independently in SQL, so each is fetched at `skip + limit` with a deterministic id tiebreak, under a 5,000-row deep-paging ceiling that refuses rather than scanning the tenant. Protected export parity remains. |
| FEE-16 | M3 | 03 | PARTIAL | Reversal/receipt history exists; canonical void register needs closure. |
| FEE-17 | M3 | 03 | PARTIAL | Receipt and sequence-exception registers use typed decimal-string contracts and server pagination, and the receipt register now carries the versioned metadata envelope. **Defect found and fixed:** `summary.totalAmount`, `totalRefundedAmount`, and `totalNetAmount` were summed from the displayed page while sitting beside a report-wide `totalReceipts`, so every figure past page one silently understated the report. They are now database aggregates over the whole filtered set, with page-only figures moved to a separately labelled `pageTotals`. Protected export parity remains. |
...
| PAY-04 | M7 | 07 | PARTIAL | Tax deductions exist, but **no report is registered** for PAY-04, so there is nothing to link to the catalog. The only registered statutory payroll exports are `statutory-tds-summary` (linked to TAX-01) and `statutory-pf-summary` (PF, which has no P0 catalog ID). Professional verification state needs closure. |
...
| TAX-01 | M7/M11 | 07 | PARTIAL | TDS data exists. The `statutory-tds-summary` export is now linked to the catalog, so it classifies `STATUTORY_DRAFT` and its PDF/XLSX artifacts carry a `STATUTORY DRAFT` marking inside the document instead of `CONFIDENTIAL`. Nepal-qualified professional verification remains external and unobtained, so the report stays a draft. The canonical metadata envelope and export parity are not yet adopted for it. |
| TAX-02 | M7/M11 | 07 | PARTIAL | TDS payable data exists but **no report is registered**, so there is nothing to link to the catalog. Nepal-qualified professional verification is external. |
...
| X-P0-02 | Accounting/fees/payroll entitlements fail closed | 01–09 | PARTIAL | Guards exist; new routes and disabled-module tests remain. **Provisioning gap found 2026-08-01:** `module.reports` is a canonical platform feature enforced by `@Entitlement('module.reports')` on `ReportsController`, but no `PlatformPlan` grants it (Enterprise Tier carries only academics, canteen, communications, finance, hr, library, transport) and the demo-tenant seed granted 15 modules while omitting it. The entire report registry — all 167 registered exports including every P0 financial report — was therefore unreachable in any seeded environment. The guard was behaving correctly; the provisioning was missing. Only the authenticated browser flow surfaced this, because unit and integration tests bypass the entitlement guard. Root cause: reports were modelled as graded features (`feature.reports.basic/standard/advanced/custom`, present on every tier) while the controller enforces a `reports` **module** that no tier declared, so `checkModuleEnabled('reports')` never matched. Fixed by adding `reports` to `CUSTOMER_MODULES` and to all four tiers in `ENTITLEMENT_MATRIX`, plus the platform plan-feature seed so database plan rows stay aligned with the matrix as that seed requires. The module now gates access to the report centre while the `feature.reports.*` keys continue to grade depth. Verified against a live API with the tenant override deleted: an Enterprise tenant resolves the report registry from its plan alone, and the FEE-01 browser flow passes plan-only. Focused guards pin every tier and prove a tenant override still fails closed. |
...
| `playwright test e2e/fee-01-student-ledger-export.spec.ts` (seeded local stack, Accountant persona) | PASS — 2/2, re-verified with plan-only entitlement after the tenant override was deleted. Export → protected download and unauthorized-role denial. Each run leaves a ReportExport row and File Registry asset in the target tenant, which `db:clean:e2e` does not sweep. |
| Focused API `src/plans src/platform src/reports src/finance src/accounting src/mobile src/payroll` Jest | PASS — 706/706 across 66 suites. |
...
| 2026-08-01 | FIN/UI-P0-02 (partial) | Ran the authenticated accounting Playwright flow for FEE-01 against the seeded local stack: Accountant → reports centre → PDF export → protected snapshot download (2/2 passing), plus an unauthorized-role negative case. Verified the resulting `ReportExport` row in PostgreSQL carries the full FEE-01 envelope and matching `displayedTotals`. The run exposed a provisioning gap recorded under X-P0-02: `module.reports` was granted by no plan and by no tenant seed, making the whole report registry unreachable in seeded environments. Added `module.reports` to the demo-tenant seed feature overrides; plan-level provisioning is still missing. Also recorded two E2E locator hazards: the topbar global search placeholder collides with a loose `/Student/i` match, and entitlement changes need a Redis flush before the API honours them. |
| 2026-08-01 | X-P0-02 | Provisioned `module.reports` at plan level, closing the blocker that made the entire report/export registry unreachable for real tenants. Added `reports` to `CUSTOMER_MODULES` and to the STARTER/STANDARD/PROFESSIONAL/ENTERPRISE tiers in `packages/core` `ENTITLEMENT_MATRIX`, and to the platform plan-feature seed. Proven live with the tenant override deleted and the Redis entitlement cache flushed: the Accountant resolves 21 reports including FEE-01 from plan entitlements alone, and the FEE-01 export → protected download browser flow passes 2/2 plan-only. Two focused guards added (every tier grants it; a tenant override still fails closed), the first mutation-checked by reverting the matrix and confirming failure. Focused API 706/706 across 66 suites. |
| 2026-08-01 | FIN/UI-P0-07 (partial) | Linked the one registered statutory export to the catalog: `statutory-tds-summary` → `TAX-01`, chosen because it returns per-deductee TDS detail rather than payroll-side deductions (PAY-04) or the payable side (TAX-02). Its exports now classify `STATUTORY_DRAFT`, and the generated PDF carries a `STATUTORY DRAFT` marking inside the document with `CONFIDENTIAL` asserted absent. PAY-04 and TAX-02 have **no registered export**, so nothing was invented for them; a focused test now pins which catalog IDs remain unlinked so the gap cannot be silently assumed closed. Statutory-link and artifact tests mutation-checked. Focused API 708/708 across 66 suites. **Divergence recorded, not changed:** `statutory-tds-summary` requires `payroll:read` while the catalog specifies `payroll:reports:read` for TAX-01. Today only `hr_manager` holds payroll permissions and it holds both, so nothing is exploitable, but a custom role granting one without the other would diverge from the P0 contract. Changing authorization on a statutory payroll report needs an owner decision. |
| 2026-08-01 | FIN/UI-P0-03 (partial) | Replicated the envelope pattern onto the receipt (FEE-17) and refund/reversal (FEE-15) registers, the first reuse of the shared `FinancialReportEnvelope` beyond FEE-01 — the helper carried over unchanged, which is the outcome the extraction was for. Fixed a real understatement in FEE-17 where page-summed money was presented as report-wide totals. Replaced FEE-15's load-everything-and-slice paging with a bounded two-source merge plus aggregate totals and an explicit deep-paging ceiling. FEE-15 had no test coverage at all; it now has totals, bounded-scan, and ceiling tests, and the bounded-scan assertion is mutation-checked. Focused API 710/710 across 66 suites. |
