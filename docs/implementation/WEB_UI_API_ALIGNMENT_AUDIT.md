...
**Scope:** School operations web routes for M1-M13, the shared authenticated dashboard shell, and contract-safe desktop-reference alignment for M3 Fees & Receipts and M5 Activity Feed & Milestones.
...
| M8 Library | `/dashboard/library`, `/books`, `/catalog`, `/copies`, `/issue-return`, `/borrowers`, `/reservations`, `/overdue`, `/fines`, `/reports` | Catalogue/copies/issues/returns/reservations/overdue/fines/reports; scanner copy lookup; protected/fine handoff routes | No single overview contract exists for all visual KPI cards. Copy-status, circulation, and inventory-health official totals must remain unavailable until a bounded Library summary exists. |
| M9 Transport | `/dashboard/transport`, `/routes`, `/vehicles`, `/assignments`, `/students`, `/trips`, `/location`, `/reports` | Routes/stops/vehicles/assignments/trips/stale-GPS/location freshness/reports; trip lifecycle and boarding reports | Parent-safe projections and driver companion workflows remain purpose-limited. Live-map claims stay deferred; operators see timestamped backend location freshness rather than implied live tracking. |
| M10 Canteen | `/dashboard/canteen`, `/menu`, `/plans`, `/wallet`, `/pos`, `/serving`, `/inventory`, `/reports` | Menu/plans/enrolments/wallet/POS/serving/inventory/suppliers/reports; QR serving and allergy acknowledgement | Stock, wallet, and serving totals must be backend-owned. Allergy and medical details stay canteen-permission scoped and are not exposed through cross-module summaries. |
| M11 Accounting | `/dashboard/accounting`, `/chart-of-accounts`, `/journals`, `/accounts`, `/reports`, `/reconciliation`, `/fiscal-periods`, `/management`, `/audit` | Accounts/fiscal periods/journals/ledger/reports/exports; M11 mapping/close controls; bank reconciliation and audit APIs | Existing dashboard is broad and real. Reference KPIs need a bounded overview response or explicit unavailable values; export-job status must remain backend job truth. |
| M12 Notices & Communication | `/dashboard/notices`, `/new`, `/deliveries`, `/[noticeId]`; `/dashboard/messages`, `/threads`, `/moderation`, `/[threadId]`; `/dashboard/messaging` | Notices/templates/audience preview/scheduling; delivery logs/analytics/failures/retry; notification center; messaging and parent-teacher chat scope/moderation | `/dashboard/communications` does not exist. Notices, deliveries, and chat can be composed from real APIs. Provider status/diagnostics and the full six-metric overview require a safe provider-health/summary contract; browser timers are prohibited. |
...
7. Keep M3 and M11 separate. M3 runs school fee collection and its operational follow-up; M11 owns accounting journals, fiscal controls, and accounting reconciliation records.
...
| Delivery queue, failed-recipient replay, provider diagnostics, templates, device logs | M12 Notices & Communication | Do not create a second delivery operations center inside M5. Retry/replay remains reasoned, provider-safe, and idempotent under M12 contracts. |
