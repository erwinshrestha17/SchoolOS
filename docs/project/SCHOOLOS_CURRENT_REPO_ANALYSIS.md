...
| M2 Attendance | Attendance sessions/records, corrections, sync submissions/conflicts, drafts, monthly/history analytics, exports, teacher-scope tests, guardian-scoped mobile attendance summary with month-history data. | Attendance daily workflow and register routes; Flutter parent attendance uses the purpose-limited mobile API and selected-child state. | Offline sync persistence hardening, attendance export/report stabilization, more guardian/tenant-isolation tests. | Slow-network/offline UX, correction workflow polish, and browser/manual QA for reconnect conflict choices. |
