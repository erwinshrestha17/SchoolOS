...
M5 Activity Feed & Milestones: Strong foundation with media privacy, consent-aware blocking, optimized previews, app-controlled private media preview/download helpers with active-child/tagged-consent direct checks, moderation controls, teacher media gallery, backend milestone template presets, backend audience preview, active-student-only tagging and delivery, retry-pending delivery replay idempotency, and media-access entitlement gating
...
M9 Accounting: Production-candidate / Pilot-Ready with Nepal school chart template preview/import, source-mapping health checks, PDFs, retry-safe report snapshot reuse, synchronous export row thresholds, queued background General Ledger/Cash Book exports through File Registry, audit trail, reconciliation suggestions, File Registry export support, and validated bank statement import DTO/service hardening
