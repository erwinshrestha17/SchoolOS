...
M4 Academics / Exams / CAS / Report Cards: Completed / Pilot-Ready with PDF/report/correction/snapshot polish, protected/app-controlled report-card PDF opening, and backend assessment template presets
M5 Activity Feed & Milestones: Strong foundation with media privacy, consent-aware blocking, optimized previews, app-controlled private media preview/download helpers, moderation controls, teacher media gallery, backend milestone template presets, backend audience preview, active-student-only delivery, retry-pending delivery replay idempotency, and media-access entitlement gating
