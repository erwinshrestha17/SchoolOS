...
High P1 register (not exhaustive; see Wave 0 report): Prisma auto-scope excludes upsert/aggregate/raw + jobs without CLS; support-override breaks `/auth/me` (**remediated M0**); post-revoke guardian residual access (**remediated M1**); primary-guardian DB constraint (**remediated M1 P1-05**); nullable-unique gaps on assignment tables; `/ready` 200-when-degraded; no monitoring/alerts; no Redis auth; shallow `verify:openapi` gate; mobile cold-start expired-token logout; mobile homework create idempotency; homework date-filter client pagination; bulk import no rollback; grading-scale UI missing; school profile address not using Nepal geography cascade.
