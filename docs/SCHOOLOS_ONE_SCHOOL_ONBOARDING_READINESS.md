...
| P0 | Tenancy | Tenant isolation (request path) | OK (JWT claim + CLS + Prisma scope injection) | OK | OK | Locally hardened | 22 PostgreSQL isolation cases + full API unit | Local only | Implemented but Unverified | P1-02 locally remediated: aggregate/groupBy/upsert/future operations fail closed or inject tenant scope; raw SQL is denied inside tenant context except explicit global probes. TLS staging and controlled-pilot evidence remain pending |
...
| P0 | Students | Bulk CSV import | OK (dry-run, batch confirm, error CSV, template download) | OK (iEMIS workspace + template download) | N/A | OK | Tests exist, Manually Unverified | None | Implemented but Unverified | CSV template via `GET /admissions/bulk-import/template` (2026-07-29); no rollback of successful rows (per-row commit); duplicate re-import blocked |
...
| P0 | Academic structure | Grading scale configuration | OK (settings key validation) | OK (structured policy editor) | N/A | OK | Unit/static web coverage | Local only | Implemented but Unverified | Structured grading bands and failing-band validation are implemented; staging workflow evidence remains pending |
...
| P0 | Attendance | Marking, locks, corrections, reports | OK | OK (homeroom-only mark pickers) | OK (homeroom class list for writes) | Fixed 2026-07-29 (DEF-02 remediated) | Unit Tested | Local HTTP verify 2026-07-29 | Implemented but Unverified | DEF-02 closed: writes require `HOMEROOM_ATTENDANCE_MARK`; web/mobile mark pickers and mobile class list homeroom-only (2026-07-29 Wave 1 slice). Evidence: `m2-attendance-core-2026-07-29-local.md`. TLS/controlled-pilot still pending |
...
| P0 | Ops | Production configuration and deployment | Partial (env validation strong) | Partial | Partial | Good env gating | Local authenticated Redis and compose rehearsal | Local only | Implemented but Unverified | Container/app compose and migrate-fail-closed startup exist; Redis auth/TLS configuration is implemented and authenticated locally, but certificate-verified TLS staging evidence remains missing |
...
| P0 | Ops | Observability (health, logging, request IDs) | Partial | N/A | N/A | Safe error envelope confirmed | Unit + local failure/recovery rehearsal | Local only | Implemented but Unverified | `/ready` returns 503 for degraded dependencies and 200 after recovery; health monitor/webhook rehearsal exists. Real host deployment, alert delivery, and log aggregation remain pending |
...
| P0 | Mobile | Shell stability (login, restore, routing) | N/A | N/A | OK | Persona route guards | Full Flutter suite + debug Android/iOS builds | Local only | Implemented but Unverified | P1-09 locally remediated: cold start rotates an expired access token, preserves tenant-scoped cached identity only for transient refresh failures, and clears rejected sessions. Physical-device/TLS staging evidence remains pending |
| P1 | Homework | Assign, review, submissions | OK | OK (server pagination/date filtering) | OK (replay-safe create) | Subject-scope enforced on writes | Focused API/web/mobile + full API/Flutter suites | Local only | Implemented but Unverified | P1-10/P1-12 locally remediated: create uses immutable replay semantics with concurrent winner recovery; web listing is server-paginated. Staging/device evidence remains pending |
| P1 | Notices | Authoring, publishing, delivery (M15→M12) | OK | OK (approvals write UI + decision panel) | OK (inbox) | P1-04 post-revoke inbox **remediated 2026-07-29** | Local HTTP verify 2026-07-29 | None | Implemented but Unverified | `verify:m15-notices` PASS: publish → M12 delivery → parent ack idempotent. Evidence: `m15-notices-core-2026-07-29-local.md` |
...
| P2 | Fees | Manual fees + receipts | OK code (idempotent, reversal patterns) | OK code | Parent views | Receipt authz tests exist | Local HTTP verify 2026-07-29 (`default-school`) | None | Disabled for Pilot (`pilot-rehearsal-1`) | `verify:m3-fees` PASS on `default-school`: cash idempotency, parent scope, local generic_json_v1 webhook proof. Evidence: `m3-fees-core-2026-07-29-local.md`. Production eSewa/Khalti TLS remains ops track. |
...
High P1 register (not exhaustive; see Wave 0 report): Prisma tenant scope (**remediated locally P1-02**); support-override `/auth/me` (**remediated M0**); post-revoke guardian residual access (**remediated M1**); primary-guardian DB constraint (**remediated M1 P1-05**); nullable-unique gaps on assignment tables; readiness degradation (**rehearsed locally P1-06**); monitoring webhook (**implemented/rehearsed locally P1-07; deployment pending**); Redis auth/TLS configuration (**implemented/rehearsed locally P1-08; TLS staging pending**); compiled OpenAPI contract gate (**remediated locally P1-16**); mobile cold-start refresh (**remediated locally P1-09; device/TLS pending**); homework create idempotency (**remediated locally P1-10; device/TLS pending**); bulk import no rollback; grading-scale editor (**implemented locally**); school profile address not using Nepal geography cascade.
